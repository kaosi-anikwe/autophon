...
WORKER_SCRIPT="$SCRIPT_DIR/upload_worker.py"
...
check_dependencies() {
    ...
    if [[ ! -f "$WORKER_SCRIPT" ]]; then
        log_error "Upload worker script not found: $WORKER_SCRIPT"
        exit 1
    fi
...
setup_logging() {
    ...
    # Set log file path
    LOG_FILE="$LOG_DIR/upload_worker.log"
    
    log_info "Logs will be written to: $LOG_FILE"
}

check_running_worker() {
    if pgrep -f "upload_worker.py" > /dev/null; then
        log_warn "Upload worker appears to be already running"
        log_warn "PID: $(pgrep -f upload_worker.py)"
...
start_worker() {
    ...
    if [[ "${1:-}" == "--daemon" ]]; then
        ...
        WORKER_PID=$!
        log_success "Upload worker started with PID: $WORKER_PID"
        log_info "Monitor logs with: tail -f $LOG_FILE"
    else
        log_info "Starting in foreground mode (Ctrl+C to stop)..."
        log_info "Processing uploads in real-time..."
...
Examples:
    $0                  # Run worker in foreground (real-time monitoring)
...
Real-time Features:
    - 100ms minimum polling interval for instant response
    - 2s maximum polling when idle
...
main() {
    case "${1:-}" in
        --help|-h)
            ...
            ;;
        --check)
            log_info "Checking setup without starting upload worker..."
            check_dependencies
            source "$APP_DIR/.env" 2>/dev/null || true
            check_environment  
            setup_logging
            log_success "Upload worker setup check completed successfully"
            exit 0
            ;;
        --daemon)
            check_dependencies
