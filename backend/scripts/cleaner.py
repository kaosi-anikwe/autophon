...
def anonymous_cleanup():
    ...
    logger.info("Anonymous Clean up...")
    with app.app_context():
        ...
        for task in anonymous_tasks:
            logger.info(f"Deleting anonymous task with ID: {task.task_id}")
...
def cleanup():
    ...
    logger.info("Cleaning up uploaded tasks...")
    with app.app_context():
        ...
        for task in uploaded_tasks:
            logger.info(f"Deleting folder {task.task_path}")
...
def mark_deleted():
    ...
    logger.info("Marking tasks with missing folders as expired...")
    with app.app_context():
        # Find uploaded non-anonymous tasks
        uploaded_tasks = Task.query.filter(
            Task.task_status == TaskStatus.UPLOADED, Task.anonymous == False
    ...
    with app.app_context():
        ...
        for task in expired_tasks:
            ...
            if trans_choice in ["exp-a", "comp-ling"]:
                ...
                logger.info(f"Deleting {trans_choice} task: {task.task_id}")
...
def delete_unknown():
    ...
    logger.info("Deleting unknown directories...")
    with app.app_context():
        ...
        if os.path.exists(UPLOADS):
            for directory in os.listdir(UPLOADS):
                if directory not in user_uuids + ["README.txt"]:
                    ...
                    logger.info(f"Deleting unknown folder: {unknown_path}")
        ...
        for task in tasks_with_held:
            ...
            for held_file in held_files:
                if held_file.file_path and os.path.exists(held_file.file_path):
                    ...
                    logger.info(f"Deleted held file: {held_file.file_path}")
...
def delete_missingpron():
    ...
    logger.info("Deleting orphaned missing pronunciation dictionaries...")
        ...
        for user_dir in os.listdir(UPLOADS):
            ...
            if os.path.exists(dicts_dir):
                for dict_file in os.listdir(dicts_dir):
                    ...
                    if task_using_dict:
                        logger.info(f"Keeping: {dict_file} for user {user_dir}")
                    else:
                        logger.info(
...
def cleanup_captchas():
    ...
    logger.info("Cleaning up expired captchas...")
    with app.app_context():
        cleaned_count = Captcha.cleanup_expired_captchas()
        logger.info(f"Deleted {cleaned_count} expired captchas")
...
def backup_data():
    ...
    with app.app_context():
        ...
        if os.path.exists(history_src):
            ...
            logger.info(f"Backed up history to {history_dst}")

        # User backup
        logger.info("Backing up users.xlsx file...")
        populate_users(
            limit=datetime.now(),
        ...
        # History backup
        logger.info("Backing up history.xlsx file...")
        populate_history(
...
def main():
    ...
    try:
        ...
        logger.info(f"Cleanup completed successfully at {datetime.now()}")

    except Exception as e:
        logger.info(f"Cleanup failed with error: {str(e)}")
        raise
