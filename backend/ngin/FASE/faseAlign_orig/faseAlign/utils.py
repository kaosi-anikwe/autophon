...
class spanish_word(object):
    ...
    x_exception = [
        ...
        "mexicana",
        "mexicanas",
    ]  # note: lots of other place/proper names could be included here
    y_exception = [
        "ay",
        "buey",
        "caray",
    ...
    def __init__(self, word, override=False, custom_phones=None):
        ...
        for key in self.syllables:
            self.syllables[key][:] = [
                re.sub("1", "NY", entry) for entry in self.syllables[key]
            ]
            self.syllables[key][:] = [
                re.sub("2", "CH", entry) for entry in self.syllables[key]
            ]
    ...
    def syllabify(self):
        ...
        for group in l:
            ...
            if group[0] == "v":
                ...
                phones = phones[
                    len(group) :
                ]  # after adding group to main dict, remove it
                ...
                if c_len == 4:
                    syllables_c[prev_syll].extend(
                        phones[:2]
                    )  # first two consonants to previous syllable
                    syllables_c[syll_n].extend(
                        phones[2 : len(group)]
                    )  # remaining phones in group to current syllable
                elif c_len == 3:
                    if (
                        cluster[1:c_len] not in spanish_word.legal_clusters
                    ):  # x-sibilant-x
                        syllables_c[prev_syll].extend(
                            phones[:2]
                        )  # first two consonants to previous syllable
                        ...
                        syllables_c[prev_syll].extend(
                            phones[:1]
                        )  # first consonant to previous syllable
                        syllables_c[syll_n].extend(
                            phones[1 : len(group)]
                        )  # remaining phones in group to current syllable
                elif c_len == 2:
                    if cluster not in spanish_word.legal_clusters:
                        ...
                        syllables_c[syll_n].extend(
                            phones[: len(group)]
                        )  # all phones to current syllable
                elif c_len == 1:
                    syllables_c[syll_n].extend(
                        phones[: len(group)]
                    )  # all phones to current syllable
                    ...
                    print(
                        "Incorrect specification of cluster: "
                        + str(cluster)
                        + " - "
                        + str(self.orth)
                    )
        ...
        for syll in syllables_c.keys():
            ...
            cv = "".join(
                [
                    "c" if p in spanish_word.consonants else "v"
                    for p in syllables_c[syll]
                ]
            )  # cv framework
            v_match = re.search("vv+", cv)  # find adjacent vowels

            if v_match is not None:
                syllables[syll_n].extend(
                    phones[: v_match.start()]
                )  # append any onset consonants to syllable
                cluster = phones[
                    v_match.start() : v_match.end()
                ]  # extract vowel cluster
                while len(cluster) > 1:
                    block = cluster[:2]  # only consider two vowels at a time
                    if (
                        (block[0] not in spanish_word.unstressed_high_vowels)
                        and (block[1] not in spanish_word.unstressed_high_vowels)
                    ) or (
                        (block[0] == block[1])
                        and (block[0] in spanish_word.unstressed_high_vowels)
                    ):  # adjacent (non-high or stressed high) or (identical high) = hiatus
                        syllables[syll_n].extend(
                            block[0]
                        )  # add first vowel to current syllable
                        ...
                        syllables[syll_n].extend(
                            block[0]
                        )  # add first vowel and try again
                        cluster = cluster[1:]
                syllables[syll_n].extend(cluster)  # catch any trailing singletons
                syllables[syll_n].extend(
                    phones[v_match.end() :]
                )  # catch any coda consonants
            else:
                syllables[syll_n].extend(
                    phones
                )  # no vowel cluster so append entire syllable
    ...
    def process_syllables(self):
        ...
        if stressed == 0:  # normal rules, no orthographic accent
            if self.orth[-1] in spanish_word.vowels + ["n", "s"]:
                ...
                )  # penultimate stress since no orthographic accent
            else:
                stressed = (
                    self.num_syllables
                )  # final stress since no orthographic accent
        ...
        for syll in syllables.keys():
            for pair in spanish_word.corr_tups2:
                match, repl = pair
                syllables[syll][:] = [
                    re.sub(match, repl, x) for x in syllables[syll]
                ]  # whole list slice
        for pair in spanish_word.corr_tups2:
            match, repl = pair
