...
class Speaker:
    ...
    def __init__(self):
        ...
        self.tiernum = (
            None  # tiernum points to phone tier = first tier for given speaker
        )
...
class VowelMean:
    ...
    def __str__(self):
        return (
            "<Means for vowel class %s:  means=%s, stdvs=%s, tokens=%s,\nnormalized:  means=%s, stdvs=%s, values:\n\tF1:  %s,\n\tF2:  %s,\n\tF3:  %s>"
            % (
                self.pc,
                self.means,
                self.stdvs,
            ...
            if isVowel(p.label):
                ...
                for sn in range(len(tg) // 2):  # sn = speaknum!
                    if (sn * 2) != speaker.tiernum:
                        ...
                        while (
                            pointer[sn] < len(tg[sn * 2 + 1])
                            and tg[sn * 2 + 1][pointer[sn]].xmin() < p.xmax
                        ):
                            # current interval for comparison
                            i = tg[sn * 2 + 1][pointer[sn]]
                            # if boundaries overlap and interval not empty
                            if (
                                ((i.xmin() <= p.xmin) or (p.xmin <= i.xmin() <= p.xmax))
                                and (
                                    (i.xmax() >= p.xmax)
                                    or (p.xmin <= i.xmax() <= p.xmax)
                                )
    ...
    for w in words:
        ...
        for i in range(len(w.phones)):
            if isVowel(w.phones[i].label):
                ...
                if (
                    prec_p and prec_p != ""
                ) or prec_p == "":  # phone is a vowel and has or has not preceding segment
    ...
    for w in words:
        ...
        for i, s in enumerate(tg[-1][i_start:]):
            ...
            if s.xmin() >= w.xmax:
                ...
                i_start += (
                    i - 2
                )  # start next iteration two intervals before, just in case
                if i_start < 0:
            ...
            if s.mark().upper() != "SP":
                if (
                    (
                        s.xmin() <= w.xmin <= s.xmax()
                        and s.xmin() <= w.xmax <= s.xmax()
                    )  # "perfect" case:  entire word contained in style tier interval
                    or (
                        w.xmin <= s.xmin() and s.xmin() <= w.xmax <= s.xmax()
                    )  # word shifted to the left relative to style tier interval
                    or (
                        s.xmin() <= w.xmin <= s.xmax() and s.xmax() <= w.xmax
                    )  # word shifted to the right relative to style tier interval
                    or (w.xmin <= s.xmin() and s.xmax() <= w.xmax)
                ):  # "worst" case:  word interval contains style tier interval
...
def checkSpeechSoftware(speechSoftware):
    ...
    if speechSoftware in ["ESPS", "esps"]:
        if os.name == "nt":
            print(
                "ERROR:  ESPS was specified as the speech analysis program, but this option is not yet compatible with Windows"
            )
            sys.exit()
        if not programExists("formant"):
            print(
                "ERROR:  ESPS was specified as the speech analysis program, but the command 'formant' is not in your path"
            )
            sys.exit()
        else:
            return "esps"
        ...
        if not (
            (PRAATPATH and programExists(speechSoftware, PRAATPATH))
            or (os.name == "posix" and programExists(speechSoftware))
            or (os.name == "nt" and programExists("praatcon.exe"))
        ):
            print(
...
def checkTextGridFile(tgFile):
    ...
    if 'File type = "' not in lines[0]:
        print(
            "ERROR:  %s does not appear to be a Praat TextGrid file (the string 'File type=' does not appear in the first line.)"
            % tgFile
        )
        sys.exit()
    ...
    if style and tg[-1].name().strip().upper() not in ["STYLE", "FOCUS"]:
        ...
        for i in range(ns):
            ...
            # speaker name must be the same for phone and word tier
                ...
                tg[phone_tier(i)].name().split(" - ")[0].strip().upper()
                != tg[word_tier(i)].name().split(" - ")[0].strip().upper()
            ):
                print(
                    "ERROR!  Speaker name does not match for tiers %i and %i."
                    % (phone_tier(i), word_tier(i))
                )
                sys.exit()
            else:
...
def extractPortion(wavFile, vowelWavFile, beg, end, soundEditor):
    ...
    if soundEditor == "sox":  # this is the default setting, since it's faster
        ...
        os.system(
            os.path.join(SOXPATH, "sox")
            + " "
            + wavFile
            + " -t wavpcm "
            + os.path.join(SCRIPTS_HOME, vowelWavFile)
            + " trim "
            + str(beg)
            + " "
            + str(end - beg)
        ...
        os.system(
            os.path.join(PRAATPATH, PRAATNAME)
            + " "
            + SCRIPTS_HOME
            + "/extractSegment.praat "
            + os.path.join(os.path.pardir, wavFile)
            + " "
            + vowelWavFile
            + " "
            + str(beg)
...
def faav(phone, formants, times, intensity):
    ...
    # vowel
    if (phone.label[:-1] in ["AY", "EY", "OW", "AW"]) or (
        phone.label[:-1] == "UW" and phone.cd == "73"
    ):
...
def getMeasurementPoint(phone, formants, times, intensity, measurementPointMethod):
    ...
    if measurementPointMethod == "third":
        ...
        print(
            "ERROR: Unsupported measurement point selection method %s"
...
def getSoundEditor():
    ...
    if (
        (SOXPATH and programExists("sox", SOXPATH))
        or (os.name == "posix" and programExists("sox"))
        or (os.name == "nt" and programExists("sox.exe"))
    ):
        soundEditor = "sox"
    elif (
        (PRAATPATH and programExists("praat", PRAATPATH))
        or (os.name == "posix" and programExists("praat"))
        or (os.name == "nt" and programExists("praatcon.exe"))
    ):
        soundEditor = "praat"
    else:
        print(
            "ERROR:  neither 'praat' ('praatcon' for Windows) nor 'sox' can be found in your path"
        )
        print(
            "One of these two programs must be available for processing the audio file"
        )
        sys.exit()
...
def getSpeakerBackground(speakername, speakernum):
    ...
    if formantPredictionMethod == "mahalanobis":
        if not speaker.sex:
            print(
                "ERROR!  Speaker sex must be defined for the 'mahalanobis' formantPredictionMethod!"
            )
            sys.exit()
...
def getVowelMeasurement(
    ...
    # via Praat:  ## NOTE:  all temp files are in the "/bin" directory!
        ...
        if formantPredictionMethod == "mahalanobis":
            ...
            while nFormants <= 6:
                os.system(
                    os.path.join(PRAATPATH, PRAATNAME)
                    + " "
                    + os.path.join(SCRIPTS_HOME, "extractFormants.praat")
                    + " "
                    + vowelWavFile
                    + " "
                    + str(nFormants)
                    ...
                    + str(maxFormant)
                    + " "
                    " " + str(windowSize) + " " + str(preEmphasis) + " burg"
                )
                lpc = praat.Formant()
        ...
        if (p.label[:-1] in ["AY", "EY", "OW", "AW"]) or (
            p.label[:-1] == "UW" and p.cd == "73"
        ):
            os.system(
                os.path.join(PRAATPATH, PRAATNAME)
                + " "
                + os.path.join(SCRIPTS_HOME, "getIntensity.praat")
                + " "
                + vowelWavFile
            )
...
def hasPrimaryStress(label):
    ...
    if (
        label[-1] == "1"
    ):  # NOTE:  this assumes that there are no empty intervals on the phone tier!
        return True
    else:
        return False
...
def isVowel(label):
    ...
    if (
        re.findall(r"^([A-Z]{2,2})\d?$", label.upper())
        and re.findall(r"^([A-Z]{2,2})\d?$", label.upper())[0] in VOWELS
    ):
        return True
    else:
        return False
...
def lennig(formants, times):
    ...
    for i in range(1, len(formants) - 1):
        c = (
            abs(formants[i][0] - formants[i - 1][0])
            + abs(formants[i][0] - formants[i + 1][0])
        ) / formants[i][0] + (
            abs(formants[i][1] - formants[i - 1][1])
            + abs(formants[i][1] - formants[i + 1][1])
        ) / formants[
            i
        ][
...
def measureVowel(
    ...
    if nSmoothing:
        ...
        if 2 * nSmoothing + 1 > len(times[0]):
            print(
                "ERROR! Vowel %s in word %s is too short to be measured with selected value for smoothing parameter."
                % (phone.label, word.transcription)
            )
            return None
        else:
    ...
    if formantPredictionMethod == "mahalanobis":
        ...
        for j in range(4):
            ...
            all_tracks.append(
                getFormantTracks(
                    poles[j], times[j], phone.xmin - padBeg, phone.xmax + padEnd
                )
            )
    ...
    if formantPredictionMethod == "mahalanobis":
        ...
        if phone.label[:-1] == "AY":
            vm.glide = detectMonophthong(
                poles[winnerIndex],
                measurementPoints[winnerIndex][0],
                measurementPoints[winnerIndex][1],
            )
            ...
            if m.tracks[2 * i] and m.tracks[2 * i + 1]:
                m.norm_tracks.append(
                    round(
                        650
                        + 150
                        * (lobanov(m.tracks[2 * i], grand_means[0], grand_stdvs[0])),
                        0,
                    )
                )  # F1
                m.norm_tracks.append(
                    round(
                        ...
                        + 420
                        * (
                            lobanov(m.tracks[2 * i + 1], grand_means[1], grand_stdvs[1])
                        ),
                        0,
                    )
                )  # F2
    ...
    for p in plotnik.PLOTNIKCODES:
        ...
        try:
            m_means[p].norm_means[0] = round(
                650
                + 150 * (lobanov(m_means[p].means[0], grand_means[0], grand_stdvs[0])),
                0,
            )
        except TypeError:
        ...
        try:
            m_means[p].norm_means[1] = round(
                1700
                + 420 * (lobanov(m_means[p].means[1], grand_means[1], grand_stdvs[1])),
                0,
            )
        except TypeError:
            ...
            try:
                m_means[p].trackmeans_norm.append(
                    (
                        ...
                        round(
                            150 * (m_means[p].trackmeans[2 * i][1] / grand_stdvs[0]), 0
                        ),
                    )
                )  # mean and stdv for F1
            except TypeError:
                m_means[p].trackmeans_norm.append(("", ""))
            try:
                m_means[p].trackmeans_norm.append(
                    (
                        ...
                        round(
                            420
                            * (m_means[p].trackmeans[2 * i + 1][1] / grand_stdvs[1]),
                            0,
                        ),
                    )
                )
...
def outputMeasurements(
    outputFormat, measurements, m_means, speaker, outputFile, outputHeader, tracks
):
    ...
    if outputFormat in ["txt", "text", "both"]:
        ...
        for vm in measurements:
            ...
            # coding
            fw.write(
                "\t".join([str(round(t, 1)) if t else "" for t in vm.tracks])
            )  # formant tracks
            ...
            if candidates:
                ...
                fw.write(
                    "\t".join(
                        [
                            ",".join([str(p) for p in vm.poles]),
                            ",".join([str(b) for b in vm.bandwidths]),
                        ]
                    )
                )
        ...
        fw.close()
        print(
            "Vowel measurements output in .txt format to the file %s"
            % (os.path.splitext(outputFile)[0] + ".txt")
        )
        ...
        for vm in measurements:
            ...
            # coding
            fw.write(
                "\t".join([str(round(t, 1)) if t else "" for t in vm.norm_tracks])
            )  # formant tracks
            fw.write("\t")
            if vm.nFormants:
                fw.write(str(vm.nFormants))
        ...
        fw.close()
        print(
            "Normalized vowel measurements output in .txt format to the file %s"
            % (os.path.splitext(outputFile)[0] + "_norm.txt")
        )
        ...
        if tracks:
            with open(os.path.splitext(outputFile)[0] + ".tracks", "w") as trackfile:
                ...
                for nmeas, vm in enumerate(measurements):
                    ...
                    b1_tracks = [
                        b[0] if len(b) >= 1 else "" for b in vm.winner_bandwidths
                    ]
                    b2_tracks = [
                        b[1] if len(b) >= 2 else "" for b in vm.winner_bandwidths
                    ]
                    b3_tracks = [
                        b[2] if len(b) >= 3 else "" for b in vm.winner_bandwidths
                    ]
                    times = vm.times[0]

                    for f1, f2, f3, b1, b2, b3, t in zip(
                        f1_tracks,
                        f2_tracks,
                        f3_tracks,
                        b1_tracks,
    ...
    if outputFormat in ["plotnik", "Plotnik", "plt", "both"]:
        ...
        plt.means = m_means
        plotnik.outputPlotnikFile(
            plt, os.path.splitext(outputFile)[0] + ".plt"
        )  # explicitly generate different extensions for "both" option
            ...
            if len(poles) >= 2:
                ...
                x = np.array(
                    [
                        poles[i],
                        poles[j],
                        math.log(bandwidths[i]),
                        math.log(bandwidths[j]),
                    ]
                )
                ...
                if len(poles) > 2:
                    ...
                    values.append(
                        [poles[i], poles[j], bandwidths[i], bandwidths[j], "", ""]
                    )
...
def programExists(program, path=""):
    ...
    if not path:
        if os.name == "posix":
            ...
            print(
                "ERROR: did not recognize OS type '%s'. Paths to 'praat' and 'sox' must be specified manually"
                % os.name
            )
            sys.exit()
        for p in pathDirs:
...
def readSpeakerFile(speakerFile):
    ...
    speaker_parser = argparse.ArgumentParser(
        description="parses a .speaker file", fromfile_prefix_chars="+"
    )
    ...
    speaker_opts_keys = [
        x
        for x in speaker_opts_dict.keys()
        if x not in ["tiernum", "speakernum", "vowelSystem"]
        and speaker_opts_dict[x] is not None
    ]
    ...
    for attribute in speaker_opts_keys:
        ...
        if hasattr(speaker, attribute):
            ...
            print(
                "WARNING!  Speaker object has not attribute %s (value %s)!"
                % (attribute, value)
            )
...
def setup_parser():
    parser = argparse.ArgumentParser(
        description="Takes as input a sound file and a Praat .TextGrid file (with word and phone tiers) and outputs automatically extracted F1 and F2 measurements for each vowel (either as a tab-delimited text file or as a Plotnik file).",
    ...
    parser.add_argument(
        "--covariances",
        "-r",
        default=pkg_resources.resource_filename("fave.extract", "config/covs.txt"),
        help="covariances, required for mahalanobis method",
    )
    parser.add_argument(
        "--formantPredictionMethod",
        choices=["default", "mahalanobis"],
        default="mahalanobis",
    ...
    parser.add_argument(
        "--means",
        "-m",
        default=pkg_resources.resource_filename("fave.extract", "config/means.txt"),
        help="mean values, required for mahalanobis method",
    )
    parser.add_argument(
        "--measurementPointMethod",
        choices=["fourth", "third", "mid", "lennig", "anae", "faav", "maxint"],
        default="faav",
        help="Method for determining measurement point",
    )
    parser.add_argument(
        "--mfa",
        action="store_true",
    ...
    parser.add_argument(
        "--outputFormat",
        "-o",
        choices=["txt", "text", "plotnik", "Plotnik", "plt", "both"],
        default="txt",
    ...
    parser.add_argument(
        "--speechSoftware",
        choices=["praat", "Praat", "esps", "ESPS"],
        default="Praat",
        help="The speech software program to be used for LPC analysis.",
    )
    parser.add_argument("--speaker", "-s", help="*.speaker file, if used")
    parser.add_argument(
        "--stopWords",
        nargs="+",
        default=[
    ...
    parser.add_argument(
        "--stopWordsFile", "-t", help="file containing words to exclude from analysis"
    )
    parser.add_argument(
        "--tracks", action="store_true", help="Write full formant tracks."
    )
    parser.add_argument(
        "--vowelSystem",
        choices=["phila", "Phila", "PHILA", "NorthAmerican", "simplifiedARPABET"],
        default="NorthAmerican",
        help="If set to Phila, a number of vowels will be reclassified to reflect the phonemic distinctions of the Philadelphia vowel system.",
    )
    parser.add_argument(
        "--verbose",
        "-v",
    ...
    parser.add_argument("wavInput", help="*.wav audio file")
    parser.add_argument("tgInput", help="*.TextGrid alignment")
    parser.add_argument("output", help="File stem for output")
...
def whichSpeaker(speakers):
    ...
    if speaknum not in range(len(speakers)):
        print(
            "ERROR!  Please select a speaker number from 1 - %i.  "
            % (len(speakers) + 1)
        )
...
def writeLog(filename, wavFile, maxTime, meansFile, covsFile, opts):
    ...
    if version:
        ...
        f.write(
            "Consider installing Git (http://git-scm.com/).\
    ...
    try:
        ...
        try:
            ...
            (
                changes,
                err,
            ) = check_changes.communicate()  # pylint: disable=unused-variable
        except OSError:
            changes = ""
    ...
    if count_vowels:
        f.write(
            "->\tNumber of vowels analyzed:\t%i\t(%.1f%%)\n"
            % (count_analyzed, float(count_analyzed) / float(count_vowels) * 100)
        )
        f.write(
            "->\tNumber of vowels discarded:\t%i\t(%.1f%%)\n"
            % (
                (count_vowels - count_analyzed),
                float((count_vowels - count_analyzed)) / float(count_vowels) * 100,
            )
        )
    f.write("\n")
    f.write("Duration of sound file:\t\t%.3f seconds\n" % maxTime)
    f.write(
        "Time for program run:\t\t%.3f seconds\n" % (logtimes[-1][1] - logtimes[0][1])
    )
    if count_analyzed:
        f.write(
            "->\t%.3f seconds per analyzed vowel\n"
...
def extractFormants(wavInput, tgInput, output, opts, SPATH="", PPATH=""):
    ...
    for wavFile, tgFile, outputFile in zip(wavFiles, tgFiles, outputFiles):
        ...
        for pre_w, w, fol_w in window(words, window_len=3):
            ...
            if (
                w.transcription == ""
                or w.transcription == "((xxxx))"
                or w.transcription.upper() == "SP"
            ):
                pbar.update(1)
                continue
            ...
            if numV == 0:
                if opts.verbose:
                    print("")
                    print(
                        "\t\t\t...no vowels in word %s at %.3f."
                        % (w.transcription, w.xmin)
                    )
                pbar.update(1)
                continue
            ...
            if uncertain.search(w.transcription):
                ...
                if opts.verbose:
                    print("")
                    print(
                        "\t\t\t...word %s at %.3f is uncertain transcription."
                        % (w.transcription, w.xmin)
                    )
                pbar.update(1)
                continue
            ...
            for p_index, p in enumerate(w.phones):
                ...
                vowelFileStem = (
                    fileStem + "_" + p.label
                )  # name of sound file - ".wav" + phone label
                vowelWavFile = vowelFileStem + ".wav"

                if opts.verbose:
                    print("")
                    print(
                        "Extracting formants for vowel %s in word %s at %.3f"
                        % (p.label, w.transcription, w.xmin)
                    )
        ...
        # write log file
        writeLog(
            os.path.splitext(outputFile)[0] + ".formantlog",
            wavFile,
            maxTime,
            meansFile,
            covsFile,
