...
class CMU_Dictionary:
    ...
    def __init__(self, dictionary_file, **kwargs):
        ...
        logging.basicConfig(format="%(levelname)s:%(message)s", level=kwargs["verbose"])
    ...
    def add_dictionary_entries(self, infile, path="."):
        ...
        for line in lines:
            try:
                ...
                transcriptions = [
                    self.check_transcription(t.strip())
                    for t in columns[1].replace('"', "").split(",")
                ]
        ...
        if os.path.exists(
            added_items_file
        ):  # check whether dictionary additions file exists already
    ...
    def check_phone(self, phone, transcription, index):
        ...
        if len(phone) == 3:
            if str(phone[-1]) not in ["0", "1", "2"]:
                self.logger.error(
                    "Unknown stress %s for vowel %s in word %s",
                    phone[-1],
                    phone,
                    transcription,
                )
            ...
            if phone[:-1] not in self.VOWELS:
                raise ValueError(
                    "Unknown vowel %s (at position %i) in word %s!\n"
    ...
    def check_word(self, word, next_word="", unknown=None, line=""):
        ...
        if not inDict and word not in self.STYLE_ENTRIES:
            ...
            if self.start_uncertain.search(word) or self.end_uncertain.search(word):
                if self.start_uncertain.search(word) and self.end_uncertain.search(
                    word
                ):
                    word = word.replace("((", "")
                    word = word.replace("))", "")
                    ...
                    error = (
                        "ERROR!  Something is wrong with the transcription of word %s!"
                        % word
                    )
...
def read_dict(f):
    ...
    for line in lines:
        ...
        if word not in dict:
            ...
            dict[word].append(
                phones
            )  # add alternative pronunciation to list of pronunciations
    dictfile.close()
    return dict
