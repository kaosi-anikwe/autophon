...
class Formant:
    ...
    def read(self, file):
        ...
        if format == "short":  # SHORT FORMANT FORMAT
            ...
            self.__xmax = round(
                float(text.readline().rstrip().split()[2]), 3
            )  # end time
...
class Intensity:
    ...
    def read(self, filename):
        ...
        if format == "short":  # SHORT FORMAT
            ...
            self.__x1 = round(
                float(text.readline().rstrip()), 3
            )  # time of first frame (x1)
            text.readline()  # (ymin)
            text.readline()  # (ymax)
            text.readline()  # (ny)
            text.readline()  # (dy)
    ...
    def read(self, filename):
        ...
        if format == "short":  # SHORT TEXTGRID FORMAT
            ...
            for i in range(m):  # loop over tiers
                ...
                if (
                    text.readline().strip()[1:-1] == "IntervalTier"
                ):  # line reads '"IntervalTier"'
                    ...
                    for j in range(n):
                        ...
                        jmrk = text.readline().strip()[
                            1:-1
                        ]  # line reads '"abcdefg"' (interval label)
            ...
            self.__xmax = round(
                float(text.readline().strip().split(" = ")[1]), 3
            ...
            for i in range(m):  # loop over tiers
                ...
                if (
                    text.readline().rstrip().split()[2] == '"IntervalTier"'
                ):  # line reads "class = 'IntervalTier"'
                    inam = (
                        text.readline().strip().split("=")[1].strip()[1:-1]
                    )  # line reads 'name = "xyz"'
                    imin = round(
                    ...
                    itier = IntervalTier(inam, imin, imax)
                    n = int(
                        text.readline().strip().split("=")[1].strip()
...
class IntervalTier:
    ...
    def extend(self, newmin, newmax):
        ...
        if newmin > self.__xmin:
            raise ValueError(
                "New minimum of tier %f exceeds old minimum %f." % (newmin, self.__xmin)
            )
        if newmax < self.__xmax:
            raise ValueError(
                "New maximum of tier %f is less than old maximum %f."
                % (newmax, self.__xmax)
            )
        ...
        while z < end:  # (only go up to second-to-last interval)
            ...
            if i.xmax() != self.__intervals[z + 1].xmin():
                ...
                if i.xmax() < self.__intervals[z + 1].xmin():
                    ...
                    print(
                        "tidyup:  Added new interval %f:%f to tier %s."
                        % (i.xmax(), self.__intervals[z + 1].xmin(), self.__name)
                    )
                    self.__n = len(self.__intervals)
                    self.sort_intervals()
