...
__author__ = (
    "Rosenfelder, Ingrid; "
    + "Fruehwald, Josef; "  # Only code writers
    + "Evanini, Keelan; "
    + "Seyfarth, Scott; "
    + "Gorman, Kyle; "
    + "Prichard, Hilary; "
...
class Aligner:
    ...
    def __init__(self, wavfile, trsfile, tgfile, **kwargs):
        self.logger = logging.getLogger(__name__)
        logging.basicConfig(
            format="%(name)s - %(levelname)s:%(message)s", level=kwargs["verbose"]
        )
        ...
        self.transcript = transcriptprocessor.TranscriptProcessor(
            self.transcript, self.cmu_dict, *args, **kwargs
        )
    ...
    def get_duration(self, FADIR="", PRAATPATH=""):
        ...
        try:
            ...
            self.logger.debug(
                "Script path is %s",
                os.path.join(FADIR, "praatScripts", "get_duration.praat"),
            )
            if PRAATPATH:
                dur_command = "%s %s %s" % (
                    PRAATPATH,
                ...
                dur_command = "praat %s %s" % (
                    os.path.join(FADIR, "praatScripts", "get_duration.praat"),
                    self.audio,
                )
            duration = round(
                float(
                    subprocess.Popen(dur_command, shell=True, stdout=subprocess.PIPE)
                    .communicate()[0]
                    .strip()
                ),
                3,
    ...
    def align(self, tempdir="", FADIR=""):
        ...
        for text, line in zip(trans_lines, all_input):
            ...
            self.__cut_chunk(os.path.join(tempdir, chunkname_sound), beg, dur, SOXPATH)
            ...
            # align chunk
            self.__align(
                os.path.join(tempdir, chunkname_sound),
                [text],
                os.path.join(tempdir, chunkname_textgrid),
                FADIR,
                SOXPATH,
                self.htktoolspath,
            )
    ...
    def __cleanup(self, style_tier, main_textgrid, failed_alignment, duration, counts):
        ...
        try:
            ...
            self.logger.error("Could not remove temporary dictionary directory!")
            raise e
        else:
        ...
        try:
            self.__write_log(
                os.path.splitext(wavfile)[0] + ".FAAVlog", wavfile, duration, counts
            )
            ...
            self.logger.debug(
                "Written log file %s.",
                os.path.basename(os.path.splitext(wavfile)[0] + ".FAAVlog"),
            )
    ...
    def __cut_chunk(self, outfile, start, dur, SOXPATH):
        ...
        try:
            ...
            self.logger.debug(f"Sound chunk {outfile} successfully extracted.")
        except Exception as e:
            self.logger.error(f"Could not extract {outfile}!")
            raise e

    # This was the main body of Jiahong Yuan's original align.py
    def __align(self, chunk, trs_input, outfile, FADIR="", SOXPATH="", HTKTOOLSPATH=""):
        ...
        identifier = re.sub(
            r"\W|_|chunk", "", os.path.splitext(os.path.split(chunk)[1])[0]
        )
        ...
        # prepare wavefile
        SR = self.__prep_wav(chunk, tempwav, SOXPATH)

        # prepare mlfile
        self.__prep_mlf(trs_input, tempmlf, identifier)
        ...
        try:
            ...
            HCopyCommand = (
                HCopy + " -T 1 -C " + modelconfig + " -S " + tempscp + " >> " + pipedest
            )
            HViteCommand = (
                HVite
                + " -T 1 -a -m -I "
            ...
            self.__aligned_to_TextGrid(tempalignedmlf, outfile, SR)
            self.logger.debug(
                "Forced alignment called successfully for file %s",
                os.path.basename(chunk),
            )
        except Exception as e:
            FA_error = "Error in aligning file %s:  %s." % (os.path.basename(chunk), e)
        ...
        for line in transcription:
            for word in line:
                ...
                if word in self.cmu_dict.cmu_dict:
                    ...
                    self.logger.warning(f"Word '{word}' not in CMU dict!")
        fw.write(".\n")
        fw.close()
    ...
    def __aligned_to_TextGrid(self, infile, outfile, SR):
        ...
        while lines[j] != ".\n":
            ...
            if SR == 11025:  # adjust rounding error for 11,025 Hz sampling rate
                ...
                # and round!
                st = round(
                    (round(float(lines[j].split()[0]) / 10.0, 0) / 1000000.0)
                    * (11000.0 / 11025.0)
                    + 0.0125,
                    3,
                )  # start time
                en = round(
                    (round(float(lines[j].split()[1]) / 10.0, 0) / 1000000.0)
                    * (11000.0 / 11025.0)
                    + 0.0125,
                    3,
                )  # end time
                ...
                st = round(
                    round(float(lines[j].split()[0]) / 10.0, 0) / 1000000.0 + 0.0125, 3
                )
                en = round(
                    round(float(lines[j].split()[1]) / 10.0, 0) / 1000000.0 + 0.0125, 3
                )
    ...
    def __reinsert_uncertain(self, tg, text):
        ...
        for n, entry in enumerate(tgwords):
            ...
            while i <= n:
                ...
                if (
                    self.uncertain.sub(r"\1", text[i]).lstrip("*")
                    not in self.cmu_dict.cmu_dict
                    and text[i] != "((xxxx))"
                ):
    ...
    def merge_textgrids(self, main_textgrid, new_textgrid, speaker, chunkname_textgrid):
        ...
        self.logger.debug(f"Successfully added {chunkname_textgrid} to main TextGrid.")
