...
class TranscriptProcessor:
    ...
    def __init__(self, transript_file, pronunciation_dictionary, *args, **kwargs):
        ...
        for word in words:
            ...
            if word == "))":  # end of uncertain transcription span
                if self.flag_uncertain:
                    ...
                    msg = (
                        "End of uncertain transcription span detected twice in a row\n"
                    )
                    msg += (
                        "No opening double parentheses for line %s."
                        % last_end_uncertain
                    )
                    raise ValueError(msg)
                continue
    ...
    def check_transcription_format(line):
        ...
        if len(entries) != 5:
            error = (
                "Incorrect format of transcription file: %i entries per line in line %s.",
                len(entries),
                line.rstrip(),
            )
