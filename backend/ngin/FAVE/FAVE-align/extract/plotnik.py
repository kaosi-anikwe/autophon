...
class PltFile:
    ...
    def __str__(self):
        return (
            """<Plotnik file for speaker %s %s (%s, %s, %s, %s years of schooling, from %s, recorded in %s) with %s tokens.>"""
            % (
                self.first_name,
                self.last_name,
                self.age,
...
class VowelMeasurement:
    ...
    F3 = ""  # third formant
    code = (
        ""  # Plotnik code for vowel class (includes phonetic environment):  "xx.xxxxx"
    )
...
def arpabet2plotnik(ac, stress, trans, prec_p, foll_p, phoneset, fm, fp, fv, ps, fs):
    ...
    # conversions
    if (prec_p not in phoneset and prec_p != "") or (
        foll_p not in phoneset and foll_p != ""
    ):
        pc = A2P[ac]
...
def cmu2plotnik_code(i, phones, trans, phoneset, speaker, vowelSystem):
    ...
    if (i + 1 == len(phones)) or (
        (i == len(phones) - 2) and (phones[i + 1].label.upper() == "SP")
    ):
    ...
    if i == 0:
        ...
        if prec_p in ["B", "P", "V", "F"]:
            ...
            i > 1
            and prec_p in ["L", "R"]
            and phones[i - 2].label
            in ["B", "D", "G", "P", "T", "K", "V", "F", "Z", "S", "SH", "TH"]
        ):
    ...
    code = arpabet2plotnik(
        re.findall(r"^([A-Z]{2,2})\d?$", phones[i].label.upper())[0],
        re.findall(r"^[A-Z]+(\d)$", phones[i].label.upper())[0],
        trans,
        prec_p,
        foll_p,
        phoneset,
...
def get_n_foll_c(i, phones):
    ...
    for p in phones[i + 1 :]:
        if is_v(p.label):
            break
        elif n == 1 and p.label in [
            "Y",
            "W",
            "R",
            "L",
        ]:  # e.g. 'figure', 'Wrigley', etc.
            break
        else:
            n += 1
    return n
...
def get_sex(line):
    ...
    if sex not in [
        "m",
        "f",
    ]:  # if contents of third data field somthing other than sex (e.g. city)
        sex = ""
    return sex
...
def outputPlotnikFile(Plt, f):
    ...
    fw.write(
        " ".join([Plt.first_name, Plt.last_name])
        + ","
        + ",".join(
            [
                Plt.age,
    ...
    for vm in Plt.measurements:
        ...
        if (
            vm.f2
        ):  # F1, F2, F3, vowel and environmental coding, stress and duration, token
            fw.write(
                ",".join(
                    [
                        str(round(vm.f1, 1)),
    ...
    for p in PLOTNIKCODES:
        fw.write(
            ",".join(
                [
                    p,
                    str(max([len(Plt.means[p].values[i]) for i in range(3)])),
                    str(Plt.means[p].means[0]),
                    str(Plt.means[p].means[1]),
                    str(Plt.means[p].means[2]),
                    str(Plt.means[p].stdvs[0]),
                    str(Plt.means[p].stdvs[1]),
                    str(Plt.means[p].stdvs[2]),
                ]
            )
        )
        fw.write(
            " <"
            + ",".join(
                [str(round(t[0], 0)) if t[0] else "" for t in Plt.means[p].trackmeans]
            )
            + ">"
        )
        fw.write("\r")
    ...
    for vm in Plt.measurements:
        ...
        if (
            ...
            fw.write(
                ",".join(
                    [
                        str(round(vm.norm_f1, 1)),
                        str(round(vm.norm_f2, 1)),
                        str(vm.norm_f3),
                        vm.code,
                        stress + "." + str(dur),
                        vm.word,
        ...
        fw.write(
            ",".join(
                [
                    p,
                    str(max([len(Plt.means[p].values[i]) for i in range(3)])),
                    str(Plt.means[p].norm_means[0]),
                    str(Plt.means[p].norm_means[1]),
                    str(Plt.means[p].norm_means[2]),
                    str(Plt.means[p].norm_stdvs[0]),
                    str(Plt.means[p].norm_stdvs[1]),
                    str(Plt.means[p].norm_stdvs[2]),
                ]
            )
        )
        fw.write(
            ...
            + ",".join(
                [
                    str(round(t[0], 0)) if t[0] else ""
                    for t in Plt.means[p].trackmeans_norm
                ]
            )
            + ">"
        )
    ...
    fw.close()
    print(
        "Normalized vowel measurements output in .pll format to the file %s"
        % (os.path.splitext(f)[0] + ".pll")
    )
...
def phila_system(i, phones, trans, fm, fp, fv, ps, fs, pc, phoneset):
    ...
    if (
        ...
        if phones[i + 1].arpa in ["M", "N", "S", "TH", "F"]:
            ...
            # tensing consonants NOT word-finally
                ...
                # (e.g. "hand", "classroom")
                if (phoneset[phones[i + 2].arpa].cvox != "0") and trans.upper() not in [
                    "CATHOLIC",
                    "CATHOLICS",
                    "CAMERA",
                ]:
        ...
        if (
            len(phones) > i + 3
            and phones[i + 1].arpa == "S"
            and phones[i + 2].arpa in ["P", "T", "K"]
            and phoneset[phones[i + 3].arpa].cvox == "0"
        ):
            if trans[-3:] not in [
                "ING",
                "IN'",
            ]:  # exclude final "-ing"/"-in'" words, e.g. "asking"
                pc = "39"
    ...
    if pc == "2" and "ARRY" in trans.upper():
        if (
            ...
            and phoneset[phones[i + 2].arpa].cvox == "0"
        ):
            pc = "39"
    ...
    if (
        len(phones) > i + 1
        and phones[i].arpa in ["EH", "AE"]
        and phones[i + 1].arpa == "R"
    ):
        if len(phones) == i + 2:  # word-final /r/
            pc = "24"
        if (
            len(phones) > i + 2 and phoneset[phones[i + 2].arpa].cvox != "0"
...
def plt_voice(fv):
    ...
    trans_dict = {"1": "voiceless", "2": "voiced"}
    try:
...
def process_measurement_line(line):
    ...
    vm.code = line.split(",")[
        3
    ]  # Plotnik vowel code (includes phonetic environment):  "xx.xxxxx"
    ...
    if len(res) > 0:  # if present:
        ...
        vm.comment = (
            temp  # why should glide only be indicated in comment, not as glide coding?
        )
        if temp == "glide":
            vm.glide = "g"
    else:
...
def process_plt_file(filename):
    ...
    while line == "":  # stripped line empty = no content
        ...
        if (
            line == ""
        ):  # if not even end-of-line character in next line, then end of file reached
            f.close()  # (added)
    ...
    Plt.first_name = headerfields[
        0
    ]  # just put the whole name into Plt.first_name and do not bother about splitting
    ...
    if (
        not fields
    ):  # it makes a difference whether we are coming here from a blank line, or from the first line of the means
        line = f.readline().strip()
        fields = line.split(",")
    ...
    if len(Plt.measurements) != int(Plt.N):
        ...
        print(
            "len(Plt.measurements) is %s; Plt.N is %s." % (len(Plt.measurements), Plt.N)
        )
        return None
    else:
        return Plt
