...
def loadfile(file):
    ...
    for line in lines:
        ...
        vm.end = line[11]
        vm.poles = [
            [float(y) for y in x.rstrip("]").lstrip("[").split(",")]
                ...
                if vowel in vowelCovs:
                    ...
                    if np.isnan(vowelCovs[vowel][0, 0]):
                        valuesList.append(
                            [
                                ...
                                float(vm.f2),
                                vm.f3,
                                math.log(float(vm.b1)),
                                math.log(float(vm.b2)),
                                vm.b3,
                                lDur,
                            ]
                        )
                    ...
                    # "real" re-measurement
                        ...
                        nFormantsList.append(
                            i + 3
                        )  # these are the formant setting used, not the actual number of formants returned
                        keepOldTracks = False
                else:
                    valuesList.append(
                        [
