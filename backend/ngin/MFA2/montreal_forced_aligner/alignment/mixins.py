...
class AlignMixin(DictionaryMixin):
            ...
            args.append(
                CompileTrainGraphsArguments(
                    ...
                    self.working_log_directory.joinpath(
                        f"compile_train_graphs.{j.id}.log"
                    ),
        ...
        for j in self.jobs:
            ...
            args.append(
                CompileInformationArguments(
                    ...
                    getattr(self, "db_string", ""),
                    self.working_log_directory.joinpath(
                        f"compile_information.{j.id}.log"
                    ),
                    log_path,
                )
            )
    ...
    def compile_train_graphs(self) -> None:
        ...
        logger.debug(
            f"Compiling training graphs took {time.time() - begin:.3f} seconds"
        )
    ...
    def get_phone_confidences(self):
        ...
        with self.session() as session:
            with tqdm(
                ...
                session.commit()
            logger.debug(
                f"Calculating phone confidences took {time.time() - begin:.3f} seconds"
            )
