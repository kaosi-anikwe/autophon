...
class CorpusAligner(AcousticCorpusPronunciationMixin, AlignMixin, FileExporterMixin):
    ...
    def analyze_alignments_arguments(self) -> List[AnalyzeAlignmentsArguments]:
        return [
            AnalyzeAlignmentsArguments(
                ...
                self.working_log_directory.joinpath(
                    f"calculate_speech_post.{j.id}.log"
                ),
                self.model_path,
                self.align_options,
            )
    ...
    def generate_pronunciations_arguments(
        ...
        return [
            GeneratePronunciationsArguments(
                ...
                getattr(self, "db_string", ""),
                self.working_log_directory.joinpath(
                    f"generate_pronunciations.{j.id}.log"
                ),
                self.model_path,
                False,
            )
    ...
    def align(self, workflow_name=None) -> None:
        ...
        try:
            ...
            for j in self.jobs:
                paths = j.construct_path_dictionary(
    ...
    def compute_pronunciation_probabilities(self):
        ...
        with mfa_open(
            self.working_log_directory.joinpath(
                "pronunciation_probability_calculation.log"
            ),
            "w",
            encoding="utf8",
            ...
            for (d_id,) in dictionaries:
                ...
                for (w,) in words:
                    ...
                    for p, c in pron_counts.items():
                        ...
                        pron_mapping[(w, p)]["probability"] = format_probability(
                            c / max_value
                        )
                ...
                initial_non_silence_count = counter.non_silence_before_counts[
                    initial_key
                ] + ((1 - silence_probability) * lambda_2)
                initial_silence_probability = format_probability(
                    initial_silence_count
                    / (initial_silence_count + initial_non_silence_count)
                )
            ...
            self.silence_probability = format_probability(
                silence_prob_sum / self.num_dictionaries
            )
            ...
            if rules:
                ...
                for r in rules:
                    ...
                    for p in non_application_query:
                        ...
                        base_sil_after_count += (
                            p.silence_following_count
                            if p.silence_following_count
                            else 0
                        )
                        base_nonsil_after_count += (
                            p.non_silence_following_count
                            if p.non_silence_following_count
                            else 0
                        )
                    ...
                    r.probability = format_probability(
                        rule_count / (rule_count + base_count)
                    )
                ...
                session.query(Pronunciation).filter(
                    ...
                    Pronunciation.generated_by_rule == True,  # noqa
                ).delete()
                session.commit()
    ...
    def collect_alignments(self) -> None:
        ...
        with self.session() as session:
            if GLOBAL_CONFIG.current_profile.use_postgres:
                session.execute(
                    sqlalchemy.text("ALTER TABLE word_interval DISABLE TRIGGER all")
                )
                session.execute(
                    sqlalchemy.text("ALTER TABLE phone_interval DISABLE TRIGGER all")
                )
                session.commit()
            workflow = (
            ...
            if workflow.alignments_collected:
                return
            max_phone_interval_id = session.query(
                sqlalchemy.func.max(PhoneInterval.id)
            ).scalar()
            if max_phone_interval_id is None:
                max_phone_interval_id = 0
            max_word_interval_id = session.query(
                sqlalchemy.func.max(WordInterval.id)
            ).scalar()
        ...
        logger.info(
            f"Collecting phone and word alignments from {workflow.name} lattices..."
        )
        with tqdm(
            total=self.num_current_utterances, disable=GLOBAL_CONFIG.quiet
        ) as pbar:
            ...
            for (
                ...
                AlignmentExtractionFunction, arguments, pbar.update
            ):
                ...
                if new_word_interval_mappings:
                    has_words = True
                if (
                    GLOBAL_CONFIG.current_profile.use_postgres
                    and phone_interval_count > 1000000
                ):
                    if has_words:
                        word_buf.seek(0)
                        cursor.copy_from(
                            word_buf, WordInterval.__tablename__, sep=",", null=""
                        )
                        word_buf.truncate(0)
                        word_buf.seek(0)
                    ...
                    phone_buf.seek(0)
                    cursor.copy_from(
                        phone_buf, PhoneInterval.__tablename__, sep=",", null=""
                    )
        ...
        with self.session() as session:
            ...
            if not GLOBAL_CONFIG.current_profile.use_postgres:
                session.execute(
                    sqlalchemy.text(
                        "INSERT INTO word_interval SELECT * from word_interval_temp"
                    )
                )
                session.execute(
                    sqlalchemy.text(
                        "INSERT INTO phone_interval SELECT * from phone_interval_temp"
                    )
                )
                session.commit()
            ...
            if (
                ...
                query = (
                    ...
                    .options(
                        subqueryload(Utterance.word_intervals).joinedload(
                            WordInterval.word
                        )
                    )
                ...
                for u in query:
                    text = [
                        ...
                        if x.word.word != self.silence_word
                        and x.workflow_id == workflow.id
                    ]
            ...
            if GLOBAL_CONFIG.current_profile.use_postgres:
                session.execute(
                    sqlalchemy.text("ALTER TABLE word_interval ENABLE TRIGGER all")
                )
                session.execute(
                    sqlalchemy.text("ALTER TABLE phone_interval ENABLE TRIGGER all")
                )
                session.commit()
    ...
    def export_textgrids(
        ...
        logger.debug(
            f"Exported TextGrids in a total of {time.time() - begin:.3f} seconds"
        )
    ...
    def export_files(
        ...
        if analysis_csv.exists():
            shutil.copyfile(
                analysis_csv,
                self.export_output_directory.joinpath("alignment_analysis.csv"),
            )
    ...
    def evaluate_alignments(
        ...
        with self.session() as session:
            ...
            reference_workflow_id = self.get_latest_workflow_run(
                reference_source, session
            ).id
            comparison_workflow_id = self.get_latest_workflow_run(
                comparison_source, session
            ).id
            update_mappings = []
            indices = []
            to_comp = []
            ...
            for u in utterances:
                ...
                comparison_phones = u.phone_intervals_for_workflow(
                    comparison_workflow_id
                )
                    ...
                    for wi in u.word_intervals:
                        ...
                        if wi.word.word_type is WordType.cutoff:
                            ...
                            comparison_phones.append(
                                CtmInterval(
                                    begin=wi.begin, end=wi.end, label=self.oov_word
                                )
                            )
