...
def phones_to_prons(
    ...
    if use_g2p:
        ...
        kaldi_text = " ".join(
            [x if word_symbol_table.member(x) else oov_word for x in words]
        )
    ...
    try:
        path_string = (
            pynini.shortestpath(lattice).project("input").string(phone_symbol_table)
        )
    except Exception:
    ...
    def _run(self) -> typing.Generator[typing.Tuple[int, int]]:
        ...
        with mfa_open(self.log_path, "w") as log_file, Session(
            ...
            job = (
                session.query(Job)
                .options(
                    joinedload(Job.corpus, innerjoin=True),
                    subqueryload(Job.dictionaries),
                )
            ...
            out_disambig = os.path.join(
                workflow.working_directory, f"{self.job_name}.disambig"
            )
            ilabels_temp = os.path.join(
                workflow.working_directory, f"{self.job_name}.ilabels"
            )
            clg_path = os.path.join(
                workflow.working_directory, f"{self.job_name}.clg.temp"
            )
            ...
            if self.use_g2p:
                ...
                for d in job.dictionaries:
                    ...
                    if self.use_g2p:
                        token_type = pywrapfst.SymbolTable.read_text(
        ...
        with mfa_open(self.log_path, "w") as log_file, Session(
            ...
            for d in job.dictionaries:
                ...
                ali_path = job.construct_path(
                    workflow.working_directory, "ali", "ark", dict_id
                )
                like_path = job.construct_path(
                    workflow.working_directory, "like", "ark", dict_id
                )
                if (
                    self.confidence
...
class FineTuneFunction(KaldiFunction):
    ...
    def setup_files(
        ...
        for row in interval_query:
            ...
            if not sox_string:
                sox_string = (
                    f'sox "{data["sound_file_path"]}" -t wav -b 16 -r 16000 - |'
                )
        ...
        with mfa_open(wav_path, "w") as f:
            for k, v in sorted(wav_data.items()):
                f.write(f"{k} {v}\n")
        with mfa_open(segment_path, "w") as f, mfa_open(
            feature_segment_path, "w"
        ) as feature_f:
    ...
    def _run(self) -> typing.Generator[typing.Tuple[int, float]]:
        ...
        with Session(self.db_engine()) as session, mfa_open(
            ...
            for k, group in self.grouped_phones.items():
                for p in group:
                    ...
                    fst.add_arc(
                        initial_state,
                        pywrapfst.Arc(
                            phone_mapping[p], group_mapping[k], 0, initial_state
                        ),
                    )
            ...
            for d_id in job.dictionary_ids:
                ...
                fmllr_path = job.construct_path(
                    workflow.working_directory, "trans", "ark", d_id
                )
                ...
                use_pitch = (
                    self.pitch_options["use-pitch"] or self.pitch_options["use-voicing"]
                )
                if use_pitch:
                ...
                for boundary_id, ctm_intervals in parse_ctm_output(
                    ...
                    if current_utterance != utterance_id:
                        interval_mapping = sorted(
                            interval_mapping, key=lambda x: x["id"]
                        )
                        ...
                        while True:
                            for i in range(len(interval_mapping) - 1):
                                if (
                                    interval_mapping[i]["end"]
                                    != interval_mapping[i + 1]["begin"]
                                ):
                                    interval_mapping[i]["end"] = interval_mapping[
                                        i + 1
                                    ]["begin"]
                            new_deletions = [
                                x["id"]
                                for x in interval_mapping
                                if x["begin"] >= x["end"]
                            ]
                            interval_mapping = [
                                x
                                for x in interval_mapping
                                if x["id"] not in new_deletions
                            ]
                    ...
                    interval_mapping.append(
                        {
                            "id": interval_id,
                            "begin": round(
                                ctm_intervals[1].begin
                                + self.segment_begins[interval_id],
                                4,
                            ),
                    ...
                    while True:
                        for i in range(len(interval_mapping) - 1):
                            if (
                                ...
                                interval_mapping[i]["end"] = interval_mapping[i + 1][
                                    "begin"
                                ]
                        new_deletions = [
                    ...
                    for pi in intervals:
                        ...
                        for i in range(frame_begin, frame_end):
                            ...
                            if alternate_label == pi.phone.phone:
                                ...
                                scores.append(
                                    phone_likes[i, top_phone_ind] - actual_score
                                )
                        average_score = statistics.mean(scores)
                        alternate_label = max(
                            alternate_labels, key=lambda x: alternate_labels[x]
                        )
                        interval_mappings.append(
                            {"id": pi.id, "phone_goodness": average_score}
                        )
    ...
    def _run(self) -> typing.Generator[typing.Tuple[int, int, str]]:
        ...
        with mfa_open(self.log_path, "w") as log_file, Session(
            ...
            for d in job.dictionaries:
                ...
                ali_path = job.construct_path(
                    workflow.working_directory, "ali", "ark", d.id
                )
                ...
                for utterance, intervals in parse_ctm_output(
                    ...
                    word_pronunciations = [
                        (x[0], " ".join(x[1])) for x in word_pronunciations
                    ]
                    word_pronunciations = [
                    ...
                    if self.for_g2p:
                        ...
                        for i, x in enumerate(word_pronunciations):
                            if i > 0 and (
                                ...
                                or word_pronunciations[i - 1][0].endswith(
                                    self.clitic_marker
                                )
                            ):
                                phones.pop(-1)
...
def compile_information_func(
    ...
    with mfa_open(arguments.log_path, "w") as log_file, mfa_open(
        align_log_path, "r"
    ) as f:
...
class AlignmentExtractionFunction(KaldiFunction):
        ...
        for interval in intervals:
            ...
            if current_pron == pronunciation:
                if (
                    ...
                    pron_id = self.pronunciation_mapping.get(
                        (cur_word[0], pronunciation), None
                    )
                actual_word_intervals.append(
                    WordCtmInterval(
                        current_word_begin,
        ...
        for interval in intervals:
            ...
            if current_pron == pronunciation:
                try:
                    if (
                        pronunciation == self.oov_phone
                        and (cur_word[0], pronunciation)
                        not in self.pronunciation_mapping
                    ):
                        pron_id = self.pronunciation_mapping[
                            (self.oov_word, pronunciation)
                        ]
                    else:
                        pron_id = self.pronunciation_mapping[
                            (cur_word[0], pronunciation)
                        ]
                except KeyError:
                    pron_id = None
                try:
    ...
    def _run(
        self,
    ) -> typing.Generator[typing.Tuple[int, List[CtmInterval], List[CtmInterval]]]:
        """Run the function"""
        align_lexicon_paths = {}
        self.phone_symbol_table = None
        with Session(self.db_engine()) as session, mfa_open(
            ...
            for d in job.dictionaries:
                ...
                pronunciations = (
                    session.query(
                        Word.word, Pronunciation.pronunciation, Pronunciation.id
                    )
                ...
                if not os.path.exists(lat_path):
                    lat_path = job.construct_path(
                        workflow.working_directory, "lat", "ark", d.id
                    )
                ali_path = job.construct_path(
                    workflow.working_directory, "ali", "ark", d.id
                )
                like_path = job.construct_path(
                    workflow.working_directory, "like", "ark", d.id
                )
                if self.transcription:
                    self.utterance_texts = {}
                    ...
                    for line in one_best_proc.stdout:
                        ...
                        text = " ".join(
                            [self.reversed_word_mapping[int(x)] for x in line]
                        )
                ...
                if self.confidence and os.path.exists(lat_path):
                    ...
                    for utterance, intervals in parse_ctm_output(
                        ...
                        if like_path.exists():
                            ...
                            for interval in intervals:
                                begin_frame = int(
                                    round(interval.begin / self.frame_shift)
                                )
