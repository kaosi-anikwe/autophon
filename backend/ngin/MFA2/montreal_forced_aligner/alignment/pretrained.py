    ...
    def setup_acoustic_model(self) -> None:
        ...
        with self.session() as session:
            ...
            dictionary = (
                ...
                .filter(Dictionary.default == True)
                .first()  # noqa
            )
            if dictionary:
            ...
            for dict_name in dict_info["names"]:
                ...
                fst_path = os.path.join(
                    self.acoustic_model.dirname, dict_name + "_align.fst"
                )
                if os.path.exists(fst_path):
                    os.makedirs(dictionary.temp_directory, exist_ok=True)
    ...
    def align_one_utterance(self, utterance: Utterance, session: Session) -> None:
        ...
        for result in func.run():
            ...
            for i, index in enumerate(phone_word_mapping):
                phone_interval_mappings[i]["word_interval_id"] = word_interval_mappings[
                    index
                ]["id"]
        ...
        ).filter(PhoneInterval.workflow_id == workflow.id).delete()
        session.query(WordInterval).filter(
            WordInterval.utterance_id == utterance.id
        ).filter(WordInterval.workflow_id == workflow.id).delete()
        session.flush()
        session.bulk_insert_mappings(
            WordInterval,
            word_interval_mappings,
