...
class ModelLoadError(ModelError):
    ...
    def __init__(self, path: typing.Union[str, Path]):
        super().__init__("")
        self.message_lines = [
            f"The archive {path} could not be parsed as an MFA model."
        ]
...
class CtmError(AlignmentError):
    ...
    def __init__(self, ctm: CtmInterval):
        MFAError.__init__(
            self, f"Error was encountered in processing CTM interval: {ctm}"
        )
...
class ModelExtensionError(ArgumentError):
    ...
    def __init__(self, name: str, model_type: str, extensions: List[str]):
        ...
        if model_type:
            extra += f" for {model_type}"
        self.message_lines = [
            f'The path "{name}" does not have the correct extensions{extra}.'
        ]
...
class ModelTypeNotSupportedError(ArgumentError):
    ...
    def __init__(self, model_type, model_types):
        ...
        if model_types:
            ...
            self.message_lines.append(
                f" Possible model types: {comma_join(model_types)}."
            )
...
class KaldiProcessingError(MFAError):
    ...
    def __init__(
        self, error_logs: List[typing.Union[Path, str]], log_file: Optional[Path] = None
    ):
        super().__init__(
    ...
    def refresh_message(self) -> None:
        ...
        if self.log_file:
            self.message_lines.append(
                f" For more details, please check {self.log_file}"
            )
