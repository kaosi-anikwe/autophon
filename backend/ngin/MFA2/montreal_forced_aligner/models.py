...
class Archive(MfaModel):
    ...
    def dump(self, path: Path, archive_fmt: str = FORMAT) -> str:
        ...
        """
        return make_archive(
            os.path.splitext(path)[0], archive_fmt, *os.path.split(self.dirname)
        )
...
class AcousticModel(Archive):
    ...
    def parameters(self) -> MetaDict:
        ...
        if (
            ...
            params["position_dependent_phones"] = self.meta.get(
    ...
    def mfcc_options(self) -> MetaDict:
        ...
        return {
            ...
            "preemphasis-coefficient": self._meta["features"].get(
    ...
    def lda_options(self) -> MetaDict:
        ...
        return {
            ...
            "splice_right_context": self._meta["features"].get(
            ...
            if not os.path.exists(meta_path):
                ...
                if self._meta["features"] == "mfcc+deltas":
                    ...
                    if "pitch" in self._meta["features"]:
                        self._meta["features"]["use_pitch"] = self._meta[
                            "features"
                        ].pop("pitch")
                if (
    ...
    def pretty_print(self) -> None:
        ...
        if "citation" in self.meta:
            configuration_data["Acoustic model"]["data"]["Citation"] = self.meta[
                "citation"
            ]
        if "train_date" in self.meta:
            configuration_data["Acoustic model"]["data"]["Train date"] = self.meta[
                "train_date"
            ]
        configuration_data["Acoustic model"]["data"]["Architecture"] = self.meta[
            "architecture"
        ]
        configuration_data["Acoustic model"]["data"]["Phone type"] = self.meta[
            "phone_type"
        ]
        configuration_data["Acoustic model"]["data"]["Features"] = {
            ...
            "Performs speaker adaptation": self.meta["features"][
    ...
    def log_details(self) -> None:
        ...
        if not os.path.exists(meta_path):
            logger.debug(
                "META.YAML DOES NOT EXIST, this may cause issues in validating the model"
            )
    ...
    def add_model(self, source: str) -> None:
        ...
        for filename in self.model_files:
            if os.path.exists(os.path.join(source, filename)):
                copyfile(
                    os.path.join(source, filename), self.dirname.joinpath(filename)
                )
    ...
    def export_model(self, destination: str) -> None:
        ...
        for filename in self.model_files:
            if os.path.exists(self.dirname.joinpath(filename)):
                copyfile(
                    self.dirname.joinpath(filename), os.path.join(destination, filename)
                )
...
class G2PModel(Archive):
    ...
    def add_sym_path(self, source_directory: Path) -> None:
        ...
        if not os.path.exists(self.grapheme_sym_path) and os.path.exists(
            ...
            copyfile(
                os.path.join(source_directory, "graphemes.txt"), self.grapheme_sym_path
            )
...
class DictionaryModel(MfaModel):
            ...
            for line in f:
                ...
                if detect_phone_set:
                    ...
                    if counts[phone_set] > 100:
                        other_sets_max = max(
                            counts[x] for x in counts if x is not phone_set
                        )
    ...
    def pretty_print(self) -> None:
        ...
        dictionary = MultispeakerDictionary(
            self.path, phone_set_type=self.phone_set_type
        )
        graphemes, phone_counts = dictionary.dictionary_setup()
        configuration_data["Dictionary"]["data"]["phones"] = sorted(
            dictionary.non_silence_phones
        )
        ...
        if self.phone_set_type.has_base_phone_regex:
            for k, v in sorted(dictionary.base_phones.items()):
                if (
                    k
                    not in configuration_data["Dictionary"]["data"][
                        "detailed_phone_info"
                    ]
                ):
                    configuration_data["Dictionary"]["data"]["detailed_phone_info"][
    ...
    def load_dictionary_paths(
        self,
    ) -> Dict[str, Tuple[DictionaryModel, typing.Set[str]]]:
        """
...
class ModelManager:
    ...
    def refresh_remote(self) -> None:
        ...
        while data_count == 100:
            ...
            for d in data:
                ...
                if (
                    model_type not in self.remote_models
                ):  # Other releases, archived, etc
                    continue
    ...
    def download_model(
        ...
        local_path = (
            ...
            .joinpath(release.download_file_name)
        )
