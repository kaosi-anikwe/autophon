...
class DatabaseImportData:
    ...
    speaker_objects: typing.List[typing.Dict[str, typing.Any]] = dataclassy.factory(
        list
    )
    file_objects: typing.List[typing.Dict[str, typing.Any]] = dataclassy.factory(list)
    text_file_objects: typing.List[typing.Dict[str, typing.Any]] = dataclassy.factory(
        list
    )
...
class PhoneType(enum.Enum):
    ...
    extra = (
        5  #: Phones not to be included generally, i.e., loaded from reference intervals
    )
...
class WordType(enum.Enum):
    ...
    cutoff = (
        6  #: Words that are cutoffs of particular words or hesitations of the next word
    )
...
class PhoneSetType(enum.Enum):
    ...
    def has_base_phone_regex(self) -> bool:
        ...
        return (
            self is PhoneSetType.IPA
            or self is PhoneSetType.ARPA
            or self is PhoneSetType.PINYIN
        )
    ...
    def diphthong_phones(self) -> typing.Set[str]:
        ...
        if self is PhoneSetType.IPA or self is PhoneSetType.PINYIN:
            ...
            if self is PhoneSetType.IPA:
                diphthongs |= {
                    x + y for x, y in itertools.product(self.glides, self.vowels)
                }
                diphthongs |= {
                    x + y for x, y in itertools.product(self.vowels, self.glides)
                }
    ...
    def triphthong_phones(self) -> typing.Set[str]:
        ...
        if self is PhoneSetType.IPA or self is PhoneSetType.PINYIN:
            triphthongs = {
                x + y + z
                for x, y, z in itertools.product(self.vowels, self.vowels, self.vowels)
            }
            if self is PhoneSetType.IPA:
                triphthongs |= {
                    x + y
                    for x, y in itertools.product(self.glides, self.diphthong_phones)
                }
                triphthongs |= {
                    x + y
    ...
    def extra_questions(self) -> typing.Dict[str, typing.Set[str]]:
        ...
        if self is PhoneSetType.ARPA:
            ...
            for i in range(1, 6):
                ...
                extra_questions[f"tone_{i}"] |= {
                    f"{x}{i}" for x in self.diphthong_phones
                }
                extra_questions[f"tone_{i}"] |= {
                    f"{x}{i}" for x in self.triphthong_phones
                }
            ...
            extra_questions["sibilants"] = add_consonant_variants(
                self.sibilants | self.affricates
            )
            ...
            extra_questions["dental"] = add_consonant_variants(
                self.dental | self.labiodental
            )
            ...
            if hist_len == 1:  # it's a bigram state...
                ...
                for word in word_list:
                    ...
                    if word == "</s>":
                        ...
                        next_state = self._get_state_for_hist(
    ...
    def export_bigram_fst(
        ...
        for state in range(len(state_to_hist)):
            ...
            if hist_len == 1:  # it's a bigram state...
                ...
                output.write(
                    f"{state} {backoff_state} {this_disambig_symbol} <eps> {cost:.3f}"
                )
        output.close()
...
class PronunciationProbabilityCounter:
    ...
    silence_following_counts: collections.Counter = dataclassy.factory(
        collections.Counter
    )
    non_silence_following_counts: collections.Counter = dataclassy.factory(
        collections.Counter
    )
    silence_before_counts: collections.Counter = dataclassy.factory(collections.Counter)
    non_silence_before_counts: collections.Counter = dataclassy.factory(
        collections.Counter
    )
    ...
    def add_counts(self, other_counter: PronunciationProbabilityCounter) -> None:
        ...
        self.non_silence_following_counts.update(
            other_counter.non_silence_following_counts
        )
