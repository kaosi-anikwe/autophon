...
def output_mapping(
    mapping: Dict[str, Any], path: Path, skip_safe: bool = False
) -> None:
    """
...
def overlap_scoring(
    ...
    label_diff = compare_labels(
        first_element.label, second_element.label, silence_phone, mapping
    )
...
def align_phones(
    ...
    for a in alignments:
        for i, sa in enumerate(a.seqA):
            ...
            if sa == "-":
                ...
                if (
                    compare_labels(
                        sa.label, sb.label, silence_phone, mapping=custom_mapping
                    )
                    > 0
                ):
                    num_substitutions += 1
    ...
    if overlap_count:
        ...
        score = None
    phone_error_rate = (num_insertions + num_deletions + (2 * num_substitutions)) / len(
        ref
    )
