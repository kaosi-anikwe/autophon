...
class PronunciationProbabilityTrainer(AcousticModelTrainingMixin, PyniniTrainerMixin):
    ...
    def output_alignment_path(self) -> Path:
        """Path to temporary file to store training data"""
        return self.working_directory.joinpath(
            f"output_{self._data_source}_alignment.txt"
        )

    def generate_pronunciations_arguments(
        self,
    ) -> typing.List[GeneratePronunciationsArguments]:
        """
    ...
    def train_g2p_lexicon(self) -> None:
        ...
        with self.worker.session() as session:
            ...
            input_files = {
                x: open(
                    os.path.join(
                        working_dir, f"input_{self.worker.dictionary_base_names[x]}.txt"
                    ),
                    "w",
                    encoding="utf8",
                    newline="",
            ...
            output_alignment_files = {
                x: open(
                    os.path.join(
                        working_dir,
                        f"output_{self.worker.dictionary_base_names[x]}_alignment.txt",
                    ),
                    "w",
                    encoding="utf8",
                    newline="",
            ...
            shutil.copyfile(
                ...
                self.working_directory.joinpath("phones.txt"),
            )
            shutil.copyfile(
            ...
            for d in dictionaries:
                ...
                os.rename(
                    d.align_lexicon_path, d.align_lexicon_path.with_suffix(".backup")
                )
    ...
    def setup(self):
        ...
        for j in self.jobs:
            for p in j.construct_path_dictionary(
                previous_directory, "ali", "ark"
            ).values():
        ...
        if wf.done:
            ...
            if self.train_g2p:
                ...
                with self.worker.session() as session:
                    ...
                    for d in dictionaries:
                        ...
                        os.rename(
                            d.lexicon_fst_path,
                            d.lexicon_fst_path.with_suffix(".backup"),
                        )
            ...
            with self.worker.session() as session:
                ...
                for d in dictionaries:
                    ...
                    new_dictionary_path = self.working_directory.joinpath(
                        f"{d.id}.dict"
                    )
                    for (
                        word,
                    ...
                    if self.silence_probabilities:
                        ...
                        final_non_silence_correction_sum += (
                            d.final_non_silence_correction
                        )
                ...
                if self.silence_probabilities:
                    ...
                    self.worker.initial_silence_probability = (
                        initial_silence_prob_sum / len(dictionaries)
                    )
                    self.worker.final_silence_correction = (
                        final_silence_correction_sum / len(dictionaries)
                    )
