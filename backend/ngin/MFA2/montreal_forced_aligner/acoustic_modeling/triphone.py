...
class TriphoneTrainer(AcousticModelTrainingMixin):
    ...
    def tree_stats_arguments(self) -> List[TreeStatsArguments]:
        ...
        """
        alignment_model_path = os.path.join(
            self.previous_aligner.working_directory, "final.mdl"
        )
        arguments = []
        for j in self.jobs:
            feat_strings = {}
            ...
            for d_id in j.dictionary_ids:
                ...
                treeacc_paths[d_id] = j.construct_path(
                    self.working_directory, "tree", "acc", d_id
                )
            arguments.append(
                TreeStatsArguments(
                    j.id,
    ...
    def _setup_tree(self, init_from_previous=False, initial_mix_up=True) -> None:
        ...
        extra_question_int_path = os.path.join(
            self.worker.phones_dir, "extra_questions.int"
        )
        ...
        with mfa_open(extra_question_int_path, "r") as inf, mfa_open(
            questions_path, "a"
        ) as outf:
            for line in inf:
                outf.write(line)
