...
class AccStatsTwoFeatsFunction(KaldiFunction):
    ...
    progress_pattern = re.compile(
        r"^LOG \(gmm-acc-stats-twofeats.* Average like for this file.*"
    )
...
class SatTrainer(TriphoneTrainer):
            ...
            arguments.append(
                AccStatsTwoFeatsArguments(
                    ...
                    self.working_log_directory.joinpath(
                        f"acc_stats_two_feats.{j.id}.log"
                    ),
                    j.dictionary_ids,
                    j.construct_path_dictionary(self.working_directory, "ali", "ark"),
                    j.construct_path_dictionary(
                        self.working_directory, "two_feat_acc", "ark"
                    ),
                    self.model_path,
                    feat_strings,
                    si_feat_strings,
    ...
    def _trainer_initialization(self) -> None:
        ...
        for j in self.jobs:
            ...
            output_paths = j.construct_path_dictionary(
    ...
    def create_align_model(self) -> None:
        ...
        with mfa_open(log_path, "w") as log_file:
            ...
            if not self.quick:
                ...
                est_command.append(
                    f"--write-occs={self.working_directory.joinpath('final.occs')}"
                )
            est_command.extend(
                [
                    self.model_path,
