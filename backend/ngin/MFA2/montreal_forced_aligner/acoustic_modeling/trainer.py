    ...
    def _run(self) -> typing.Generator[typing.Tuple[int, str]]:
        ...
        with mfa_open(self.log_path, "w") as log_file, Session(
            ...
            for dict_id in job.dictionary_ids:
                ...
                tacc_path = job.construct_path(
                    workflow.working_directory, "t", "acc", dict_id
                )
...
class TrainableAligner(TranscriberMixin, TopLevelMfaWorker, ModelExporterMixin):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.training_configs: Dict[
            str,
            typing.Union[AcousticModelTrainingMixin, PronunciationProbabilityTrainer],
        ] = {}
        ...
        with self.session() as session:
            ...
            for i, (identifier, config) in enumerate(self.training_configs.items()):
                ...
                if identifier not in workflows:
                    ...
                    self.create_new_current_workflow(
                        WorkflowType.alignment, name=ali_identifier
                    )
                else:
    ...
    def add_config(self, train_type: str, params: MetaDict) -> None:
        ...
        if train_type == "monophone":
            p = {
                k: v
                for k, v in p.items()
                if k in MonophoneTrainer.get_configuration_parameters()
            }
            ...
            p = {
                k: v
                for k, v in p.items()
                if k in TriphoneTrainer.get_configuration_parameters()
            }
            ...
            p = {
                k: v
                for k, v in p.items()
                if k in LdaTrainer.get_configuration_parameters()
            }
            ...
            p = {
                k: v
                for k, v in p.items()
                if k in SatTrainer.get_configuration_parameters()
            }
            ...
            previous_trainer = self.training_configs[
                list(self.training_configs.keys())[-1]
            ]
        ...
        try:
            ...
            for j in self.jobs:
                for dict_id in j.dictionary_ids:
                    t_accs.append(
                        j.construct_path(self.working_directory, "t", "acc", dict_id)
                    )
            ...
            for line in show_proc.stdout:
                line = line.strip()
                m = re.match(
                    r"^Transition-state.*phone = (?P<phone>[^ ]+) .*pdf = (?P<pdf>\d+)$",
                    line,
                )
                if m:
                    phone = m.group("phone")
            ...
            with mfa_open(
                self.working_directory.joinpath("phone_pdf.counts"), "w"
            ) as f:
                json.dump(phone_pdf_mapping, f, ensure_ascii=False)
            logger.debug(
                f"Accumulating transition stats took {time.time() - begin:.3f} seconds"
            )
    ...
    def working_directory(self) -> Optional[Path]:
        """Working directory"""
        if (
            self.current_trainer is not None
            and not self.current_trainer.training_complete
        ):
