...
@click.argument(
    "input_path",
    type=click.Path(
        exists=True, file_okay=True, dir_okay=True, path_type=Path, allow_dash=True
    ),
)
@click.argument("g2p_model_path", type=click.UNPROCESSED, callback=validate_g2p_model)
@click.argument(
    "output_path",
    type=click.Path(file_okay=True, dir_okay=True, path_type=Path, allow_dash=True),
)
@click.option(
    "--config_path",
    "-c",
...
def g2p_cli(context, **kwargs) -> None:
    ...
    if input_path.is_dir():
        ...
        g2p = PyniniConsoleGenerator(
            g2p_model_path=g2p_model_path,
            **PyniniWordListGenerator.parse_parameters(
                config_path, context.params, context.args
            ),
        )
    else:
