...
def validate_model_arg(name: str, model_type: str) -> Path:
    ...
    if model_type not in MODEL_TYPES:
        raise click.BadParameter(
            str(ModelTypeNotSupportedError(model_type, MODEL_TYPES))
        )
...
def check_databases(db_name: str) -> None:
    ...
    GLOBAL_CONFIG.load()
    logger.debug(
        f"Checking the {GLOBAL_CONFIG.current_profile_name} MFA database server..."
    )
    ...
    try:
        ...
        with engine.connect():
            pass
        logger.debug(
            f"Connected to {GLOBAL_CONFIG.current_profile_name} MFA database server!"
        )
    except Exception:
        raise DatabaseError()
...
def initialize_server() -> None:
    ...
    logger = logging.getLogger("mfa")
    logger.info(
        f"Initializing the {GLOBAL_CONFIG.current_profile_name} MFA database server..."
    )
...
def start_server() -> None:
    ...
    try:
        check_server()
        logger.info(
            f"{GLOBAL_CONFIG.current_profile_name} MFA database server already running."
        )
        return
    except Exception:
        pass
    ...
    logger.info(
        f"Starting the {GLOBAL_CONFIG.current_profile_name} MFA database server..."
    )
    log_path = GLOBAL_CONFIG.root_temporary_directory.joinpath(
        f"pg_log_{GLOBAL_CONFIG.current_profile_name}.txt"
    )
...
def stop_server(mode: str = "smart") -> None:
    ...
    if not db_directory.exists():
        ...
        sys.exit(1)
    logger.info(
        f"Stopping the {GLOBAL_CONFIG.current_profile_name} MFA database server..."
    )
    proc = subprocess.Popen(
...
def delete_server() -> None:
    ...
    if db_directory.exists():
        logger.info(
            f"Deleting the {GLOBAL_CONFIG.current_profile_name} MFA database server..."
        )
