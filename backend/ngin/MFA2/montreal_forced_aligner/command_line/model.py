...
def inspect_model_cli(model_type: str, model: str) -> None:
    ...
    GLOBAL_CONFIG.current_profile.temporary_directory = (
        get_temporary_directory().joinpath("model_inspect")
    )
    ...
    if model_type:
        if model_type == MODEL_TYPES["dictionary"]:
            m = MODEL_TYPES[model_type](
                model, working_dir, phone_set_type=PhoneSetType.AUTO
            )
        else:
...
@click.argument(
    "new_pronunciations_path", type=click.UNPROCESSED, callback=validate_dictionary
)
@click.help_option("-h", "--help")
@common_options
@click.pass_context
