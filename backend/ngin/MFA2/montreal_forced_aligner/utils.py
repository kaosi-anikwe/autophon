...
def parse_ctm_output(
    ...
    for line in proc.stdout:
        ...
        if not line:
            continue
        try:
            utt, interval = process_ctm_line(
                line, reversed_phone_mapping, raw_id=raw_id
            )
        except ValueError:
            continue
...
def check_third_party():
    ...
    for fn in canary_kaldi_bins:
        try:
            p = subprocess.run(
                [thirdparty_binary(fn), "--help"], capture_output=True, text=True
            )
...
def log_kaldi_errors(error_logs: List[str]) -> None:
    ...
    """
    logger.debug(
        f"There were {len(error_logs)} kaldi processing files that had errors:"
    )
