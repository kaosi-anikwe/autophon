...
class Segmenter(
    ...
    def segment_vad_mfa(self) -> None:
        ...
        with tqdm(
            ...
            for utt, segments in run_kaldi_function(
                SegmentVadFunction, arguments, pbar.update
            ):
                old_utts.add(utt)
    ...
    def export_files(
        self, output_directory: str, output_format: Optional[str] = None
    ) -> None:
        """
        ...
        with self.session() as session:
            for f in session.query(File).options(
                selectinload(File.utterances).joinedload(
                    Utterance.speaker, innerjoin=True
                ),
...
class TranscriptionSegmenter(
    TranscriberMixin, SpeechbrainSegmenterMixin, TopLevelMfaWorker
):
