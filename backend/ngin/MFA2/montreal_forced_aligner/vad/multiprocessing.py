    ...
    for i, w in enumerate(words):
        ...
        if i != 0:
            fst.add_arc(
                start_state,
                pywrapfst.Arc(
                    label, label, pywrapfst.Weight.one(fst.weight_type()), next_state
                ),
            )
        fst.add_arc(
            i,
        ...
        for interjection in interjection_words:
            ...
            if " " in interjection:
                ...
                for j, iw in enumerate(i_words):
                    ...
                    fst.add_arc(
                        prev_state,
                        pywrapfst.Arc(label, label, weight, next_interjection_state),
                    )
...
def segment_utterance(
    ...
    normalized_text = " ".join(
        [x if x not in oovs else dictionary.oov_word for x in text.split()]
    )
    ...
    query = session.query(
        Word.word, Word.mapping_id, Word.initial_cost, Word.final_cost
    ).filter(Word.dictionary_id == dictionary.id)
    initial_costs = {}
    final_costs = {}
    ...
    with open(
        working_directory.joinpath("utterance.text_fst"), "w", encoding="utf8"
    ) as f:
    ...
    with mfa_open(log_path, "w") as log_file:
        ...
        ctm_proc.wait()
        split_texts = align_text(
            split_texts, text, oovs, dictionary.oov_word, interjection_words
        )
...
def segment_utterance_vad_speech_brain(
    ...
    boundaries = vad_model.remove_short_segments(
    ...
    if segmentation_options["double_check"]:
        boundaries = vad_model.double_check_speech_segments(
            boundaries,
            sound_file.sound_file_path,
            speech_th=segmentation_options["speech_th"],
        )
