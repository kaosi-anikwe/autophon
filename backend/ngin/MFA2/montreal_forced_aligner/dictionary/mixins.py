...
class SplitWordsFunction:
    ...
    def split_clitics(
        ...
        for seg in s:
            ...
            split.extend(
                [clean_initial_quote_regex.sub("", x) for x in initial_clitics]
            )
            seg = clean_final_quote_regex.sub(
                "", clean_initial_quote_regex.sub("", seg)
            )
            if seg:
                split.append(seg)
...
class DictionaryMixin:
    ...
    def silence_csl(self) -> str:
        ...
        """
        return ":".join(
            map(str, (self.phone_mapping[x] for x in self.kaldi_silence_phones))
        )
    ...
    def non_silence_csl(self) -> str:
        ...
        """
        return ":".join(
            map(str, (self.phone_mapping[x] for x in self.kaldi_non_silence_phones))
        )
...
class TemporaryDictionaryMixin(DictionaryMixin, DatabaseMixin, metaclass=abc.ABCMeta):
    ...
    def _write_word_boundaries(self) -> None:
        ...
        with mfa_open(boundary_path, "w") as f, mfa_open(
            self.word_boundary_int_path, "w"
        ) as intf:
            if self.position_dependent_phones:
                for p in sorted(
    ...
    def _write_topo(self) -> None:
        ...
        for phone_list in topo_phones.values():
            ...
            non_silence_lines = [
                ...
                "<ForPhones>",
                " ".join(
                    str(self.phone_mapping[x])
                    for x in self._generate_phone_list(phone_list)
                ),
                "</ForPhones>",
            ]
    ...
    def _write_phone_sets(self) -> None:
        ...
        roots_int_file = self.dictionary_output_directory.joinpath(
            "phones", "roots.int"
        )

        with mfa_open(sets_file, "w") as setf, mfa_open(
            roots_file, "w"
            ...
            if self.shared_silence_phones:
                ...
                for sp in self.silence_phones:
                    ...
                    phone_int_string = " ".join(
                        str(self.phone_mapping[x]) for x in mapped
                    )
                    setf.write(f"{phone_string}\n")
                    setintf.write(f"{phone_int_string}\n")
    ...
    def _write_disambig(self) -> None:
        ...
        with self.session() as session, mfa_open(disambig, "w") as outf, mfa_open(
            ...
            disambiguation_symbols = session.query(
                Phone.mapping_id, Phone.kaldi_label
