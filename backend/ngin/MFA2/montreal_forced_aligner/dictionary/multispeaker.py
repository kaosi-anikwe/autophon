...
class MultispeakerDictionaryMixin(TemporaryDictionaryMixin, metaclass=abc.ABCMeta):
        ...
        if self.phone_groups_path is not None and self.phone_groups_path.exists():
            with mfa_open(self.phone_groups_path) as f:
                ...
                if isinstance(self._phone_groups, list):
                    self._phone_groups = {
                        k: v for k, v in enumerate(self._phone_groups)
                    }
                for k, v in self._phone_groups.items():
                    self._phone_groups[k] = [
                        x for x in v if x in self.non_silence_phones
                    ]
            found_phones = set()
    ...
    def speaker_mapping(self) -> typing.Dict[str, int]:
        ...
        if not self._speaker_mapping:
            with self.session() as session:
                self._speaker_mapping = {
                    x[0]: x[1]
                    for x in session.query(Speaker.name, Speaker.dictionary_id)
                }
    ...
    def dictionary_setup(self) -> Tuple[typing.Set[str], collections.Counter]:
        ...
        with self.session() as session:
            ...
            if len(self.clitic_markers) >= 1:
                ...
                if other_clitic_markers:
                    ...
                    if "-" in other_clitic_markers:
                        extra = "-"
                        other_clitic_markers = [
                            x for x in other_clitic_markers if x != "-"
                        ]
                    clitic_cleanup_regex = re.compile(
            ...
            for (
                ...
                if (
                    dictionary_model.path not in dictionary_id_cache
                    and not self.use_g2p
                ):
                    word_cache = {}
                    ...
                    for (
                        ...
                        if " " in word:
                            logger.debug(
                                f'Skipping "{word}" for containing whitespace.'
                            )
                            continue
                        ...
                        if not pretrained and word_objs[word_cache[word] - 1][
                            "word_type"
                        ] in {
                            WordType.speech,
                            WordType.clitic,
                            ...
                            speaker_objs.append(
                                {
                                    ...
                                    "name": speaker,
                                    "dictionary_id": dictionary_id_cache[
                                        dictionary_model.path
                                    ],
                                }
                            )
                ...
                for p_id, pron in (
                    ...
                    if pron in subsequences:
                        ...
                        update_pron_objs.append(
                            {"id": p_id, "disambiguation": last_used[pron]}
                        )
    ...
    def apply_phonological_rules(self) -> None:
        ...
        with self.session() as session:
                        ...
                        for p in w.pronunciations:
                            ...
                            while True:
                                ...
                                for r in rules:
                                    n = r.apply_rule(s)
                                    if any(
                                        x not in self.non_silence_phones
                                        for x in n.split()
                                    ):
                                        continue
                                    if (
                                        n
                                        and n not in existing_prons
                                        and n not in new_variants
                                    ):
            ...
            if new_pron_objs:
                session.execute(
                    sqlalchemy.insert(Pronunciation.__table__), new_pron_objs
                )
    ...
    def _write_probabilistic_fst_text(
        ...
        if self.initial_silence_probability:
            ...
            initial_non_silence_cost = -1 * math.log(
                1.0 - self.initial_silence_probability
            )
        final_silence_cost = "0"
        final_non_silence_cost = "0"
        if (
            self.final_silence_correction is not None
            and self.final_non_silence_correction is not None
        ):
        ...
        with mfa_open(path, "w") as outf:
            ...
            for row in pronunciation_query:
                ...
                if silence_after_probability is not None:
                    ...
                    non_silence_following_cost = -math.log(
                        1 - silence_after_probability
                    )
                ...
                non_silence_before_correction = data.get(
                    "non_silence_before_correction", None
                )
    ...
    def _write_align_lexicon(
        ...
        """
        fst = pynini.Fst()
        phone_symbol_table = pywrapfst.SymbolTable.read_text(
            self.phone_symbol_table_path
        )
        word_symbol_table = pywrapfst.SymbolTable.read_text(
            dictionary.words_symbol_path
        )
        ...
        for w, pron in pronunciation_query:
            ...
            for i in range(len(pron) - 1):
                ...
                fst.add_arc(
                    current_state,
                    pywrapfst.Arc(
                        p_s, w_s, pywrapfst.Weight.one(fst.weight_type()), next_state
                    ),
                )
            ...
            fst.add_arc(
                current_state,
                pywrapfst.Arc(
                    p_s, w_s, pywrapfst.Weight(fst.weight_type(), sil_cost), sil_state
                ),
            )
        ...
        with self.session() as session:
            ...
            if rules:
                ...
                with mfa_open(output_rules_path, "w") as f:
                    yaml.dump(
                        dict(dialectal_rules), f, Dumper=yaml.Dumper, allow_unicode=True
                    )

    def add_words(
        self,
        new_word_data: typing.List[typing.Dict[str, typing.Any]],
        ...
        with self.session() as session:
            ...
            for data in new_word_data:
                ...
                if "probability" in data and data["probability"] is not None:
                    ...
                    d["non_silence_before_correction"] = data[
    ...
    def export_lexicon(
        ...
        with mfa_open(path, "w") as f:
            for data in self.words_for_export(
                dictionary_id, write_disambiguation, probability
            ):
                phones = data["pronunciation"]
    ...
    def phone_mapping(self) -> Dict[str, int]:
        ...
        if self._phone_mapping is None:
            with self.session() as session:
                phones = (
                    session.query(Phone.kaldi_label, Phone.mapping_id)
                    .order_by(Phone.id)
                    .all()
                )
    ...
    def find_all_cutoffs(self) -> None:
        ...
        with self.session() as session:
            ...
            for d_id in self.dictionary_lookup.values():
                ...
                words = (
                    session.query(
                        Word.mapping_id, Word.word, Pronunciation.pronunciation
                    )
                    .join(Pronunciation.word)
            ...
            for u_id, dict_id, normalized_text in utterances:
                ...
                for i, word in enumerate(text):
                    ...
                    if (
                        ...
                        or self.optional_silence_phone
                        in pronunciation_mapping[dict_id][next_word]
                    ):
                        continue
    ...
    def write_lexicon_information(
        self, write_disambiguation: Optional[bool] = False
    ) -> None:
        """
        ...
        with self.session() as session:
            ...
            for d in dictionaries:
                ...
                self._write_probabilistic_fst_text(
                    session, d, silence_disambiguation_symbol
                )
...
class MultispeakerDictionary(MultispeakerDictionaryMixin):
    ...
    def output_directory(self) -> str:
        ...
        return GLOBAL_CONFIG.current_profile.temporary_directory.joinpath(
            self.identifier
        )
