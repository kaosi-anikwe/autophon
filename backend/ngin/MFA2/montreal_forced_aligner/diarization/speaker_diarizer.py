...
class SpeakerDiarizer(IvectorCorpusMixin, TopLevelMfaWorker, FileExporterMixin):
        ...
        try:
            ...
            if self.evaluation_mode:
                ...
                with self.session() as session:
                    query = session.query(
                        Utterance.id, Utterance.speaker_id, Speaker.name
    ...
    def classify_speakers(self):
        ...
        with self.session() as session, tqdm(
            ...
            writer = csv.DictWriter(
                f, ["utt_id", "file", "begin", "end", "speaker", "score"]
            )

            writer.writeheader()
            file_names = {
                k: v
                for k, v in session.query(Utterance.id, File.name).join(Utterance.file)
            }
            utterance_times = {
                k: (b, e)
                for k, b, e in session.query(
                    Utterance.id, Utterance.begin, Utterance.end
                )
            }
            utterance_mapping = []
            ...
            if (
                session.query(Speaker).filter(Speaker.name == "MFA_UNKNOWN").first()
                is None
            ):
            ...
            if self.use_xvector:
                arguments = [
                    SpeechbrainArguments(
                        j.id, self.db_string, None, self.cuda, self.cluster
                    )
                    for j in self.jobs
                ]
    ...
    def map_speakers_to_ground_truth(self):
        with self.session() as session:
            ...
            ground_truth = np.array(
                [self.ground_truth_utt2spk[x] for x in utterance_ids]
            )
            cluster_labels = np.unique(labels)
            ground_truth_labels = np.unique(ground_truth)
            cm = np.zeros(
                (cluster_labels.shape[0], ground_truth_labels.shape[0]), dtype="int16"
            )
            for y_pred, y in zip(labels, ground_truth):
                if y_pred < 0:
                    continue
                cm[
                    np.where(cluster_labels == y_pred),
                    np.where(ground_truth_labels == y),
                ] += 1
    ...
    def evaluate_clustering(self) -> None:
        ...
        with self.session() as session, mfa_open(
            ...
            ground_truth_labels = np.array(
                [v for v in self.ground_truth_utt2spk.values()]
            )
            ...
            logger.info(
                f"Adjusted Rand index score (0-1, higher is better): {rand_score:.4f}"
            )
            logger.info(
                f"Normalized Mutual Information score (perfect=1.0): {nmi_score:.4f}"
            )
            logger.info(
                f"Adjusted Mutual Information score (perfect=1.0): {ami_score:.4f}"
            )
            logger.info(
                f"Homogeneity score (0-1, higher is better): {homogeneity_score:.4f}"
            )
            logger.info(
                f"Completeness score (0-1, higher is better): {completeness_score:.4f}"
            )
            logger.info(
                f"V measure score (0-1, higher is better): {v_measure_score:.4f}"
            )
            logger.info(
                f"Fowlkes-Mallows score (0-1, higher is better): {fm_score:.4f}"
            )
    ...
    def visualize_clusters(self, ivectors, cluster_labels=None):
        ...
        points = visualize_clusters(
            ivectors, self.manifold_algorithm, metric, 10, self.plda
        )
        fig = plt.figure(1)
        ...
        if cluster_labels is not None:
            ...
            for cluster in unique_labels:
                ...
                ax.scatter(
                    points[idx, 0], points[idx, 1], color=color, label=name, alpha=alpha
                )
        else:
        ...
        plt.savefig(
            plot_path, bbox_extra_artists=(lgd,), bbox_inches="tight", transparent=True
        )
        ...
        with tqdm(total=self.num_utterances, disable=GLOBAL_CONFIG.quiet) as pbar:
            arguments = [
                ExportIvectorsArguments(
                    ...
                    self.db_string,
                    j.construct_path(
                        self.working_log_directory, "export_ivectors", "log"
                    ),
                    self.use_xvector,
                )
                for j in self.jobs
        ...
        with self.session() as session:
            ...
            for s_id, f_id in query:
                speaker_ordering_mapping.append(
                    {"speaker_id": s_id, "file_id": f_id, "index": 10}
                )
    ...
    def initialize_mfa_clustering(self):
        ...
        with self.session() as session:
            ...
            utt2spk = {
                k: v for k, v in session.query(Utterance.id, Utterance.speaker_id)
            }
            with tqdm(total=self.num_utterances, disable=GLOBAL_CONFIG.quiet) as pbar:
                for utt_id, classified_speaker, score in run_kaldi_function(
                    func, arguments, pbar.update
                    ...
                    if score < score_threshold:
                        ...
                        utterance_mapping.append(
                            {
                                "id": utt_id,
                                "speaker_id": existing_speakers["MFA_UNKNOWN"],
                            }
                        )
                        continue
            ...
            session.commit()
            session.execute(
                sqlalchemy.text("DROP INDEX IF EXISTS ix_utterance_speaker_id")
            )
            session.execute(
                sqlalchemy.text("DROP INDEX IF EXISTS utterance_position_index")
            )
            session.commit()
            bulk_update(session, Utterance, utterance_mapping)
            session.execute(
    ...
    def export_speaker_ivectors(self):
        ...
        with self.session() as session, tqdm(
            ...
            speakers = (
                session.query(
                    Speaker.id, ivector_column, sqlalchemy.func.count(Utterance.id)
                )
    ...
    def classify_iteration(self, iteration=None) -> None:
        ...
        if iteration is not None and self.min_cluster_size:
            low_count = np.linspace(0, self.min_cluster_size, self.max_iterations)[
                iteration
            ]
        ...
        with self.session() as session, tqdm(
            ...
            logger.debug(
                f"Updating {len(utterance_mapping)} utterances with new speakers"
            )
    ...
    def breakup_large_clusters(self):
        with self.session() as session:
            ...
            sq = (
                session.query(
                    Speaker.id, sqlalchemy.func.count().label("utterance_count")
                )
                .join(Speaker.utterances)
            ...
            for s_id, utterance_count in sq:
                if (
                    threshold
                    and utterance_count > threshold
                    and s_id not in self.single_clusters
                ):
            ...
            with tqdm(
                ...
                for s_id in above_threshold_speakers:
                    ...
                    query = session.query(Utterance.id, Utterance.plda_vector).filter(
                        Utterance.plda_vector != None,
                        Utterance.speaker_id == s_id,  # noqa
                    )
                    pbar.update(1)
                    ...
                    for i, utt_id in enumerate(utterance_ids):
                        ...
                        utterance_mapping.append(
                            {"id": utt_id, "speaker_id": speaker_id}
                        )
                if new_speakers:
    ...
    def cleanup_empty_speakers(self, threshold=None):
        with self.session() as session:
            ...
            session.execute(
                sqlalchemy.delete(Speaker).where(~Speaker.id.in_(non_empty_speakers))
            )
            session.commit()
        ...
        try:
            ...
            conn.execute(
                sqlalchemy.text(
                    f"ANALYZE {Speaker.__tablename__}, {Utterance.__tablename__}"
                )
            )
        finally:
            conn.close()
    ...
    def cluster_utterances_mfa(self) -> None:
        ...
        with self.session() as session:
            if (
                ...
                session.add(
                    Speaker(id=self.get_next_primary_key(Speaker), name="MFA_UNKNOWN")
                )
                session.commit()
    ...
    def cluster_utterances(self) -> None:
        ...
        if self.evaluation_mode and GLOBAL_CONFIG.current_profile.debug:
            self.calculate_eer()
        logger.info(
            "Clustering utterances (this may take a while, please be patient)..."
        )
        with self.session() as session:
            if self.use_pca:
                query = session.query(Utterance.id, Utterance.plda_vector).filter(
                ...
                if cluster_id < 0:
                    if unknown_speaker_id is None:
                        ...
                        speaker_mapping.append(
                            {"id": next_speaker_id, "name": speaker_name}
                        )
            ...
            if unknown_speaker is not None:
                ...
                for utterance_id, file_id in query:
                    utterance_mapping.append(
                        {
                            "id": utterance_id,
                            "speaker_id": speaker_mapping[file_id]["id"],
                        }
                    )
    ...
    def refresh_speaker_vectors(self) -> None:
        ...
        with self.session() as session, tqdm(
            ...
            for (s_id,) in speakers:
                query = session.query(ivector_column).filter(
