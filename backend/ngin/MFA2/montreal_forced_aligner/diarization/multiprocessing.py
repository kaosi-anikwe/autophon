...
def visualize_clusters(
    ...
    logger.debug(
        f"Generating 2D representation of ivectors with {manifold_algorithm.name}..."
    )
    begin = time.time()
    to_fit = ivectors
...
def calculate_distance_threshold(
    ...
    """
    logger.debug(
        f"Calculating distance threshold from {min_samples} nearest neighbors..."
    )
    ...
    kneedle = kneed.KneeLocator(
        np.arange(distances.shape[0]), distances, curve="concave", S=5
    )
    index = kneedle.elbow
    ...
    if GLOBAL_CONFIG.current_profile.debug and not no_visuals:
        ...
        if working_directory is not None:
            plot_path = os.path.join(
                working_directory, "nearest_neighbor_distances.png"
            )
...
def cluster_matrix(
    ...
    **kwargs,
    ...
    if score_metric == "plda" and cluster_type is not ClusterType.affinity:
        ...
        logger.debug(
            f"Precomputed distance matrix took {time.time() - begin:.3f} seconds"
        )
    ...
    if cluster_type is ClusterType.affinity:
        ...
        c_labels = cluster.AgglomerativeClustering(
            metric=score_metric, **kwargs
        ).fit_predict(to_fit)
        ...
        raise NotImplementedError(
            f"The cluster type '{cluster_type}' is not supported."
        )
...
class PldaClassificationFunction(KaldiFunction):
    ...
    def _run(self) -> typing.Generator[typing.Tuple[int, int, int]]:
        ...
        speaker_ivectors = self.plda.process_ivectors(
            speaker_ivectors, counts=speaker_counts
        )
        ...
        with Session(self.db_engine()) as session:
            ...
            for u_id, u_ivector in utterances:
                ind, score = classify_plda(
                    u_ivector.astype("float64"), *classification_args
                )
            ...
            for (s_id,) in speakers:
                ...
                for u_id, s_id, u_ivector in random_within_speaker:
                    comp_query = (
                        ...
                        .filter(
                            filter, Utterance.speaker_id == s_id, Utterance.id != u_id
                        )
                        .order_by(sqlalchemy.func.random())
                        .limit(self.limit_within_speaker)
                    )
...
class SpeechbrainEmbeddingFunction(KaldiFunction):
    ...
    def _run(self) -> typing.Generator[typing.Tuple[int, int, int]]:
        ...
        while True:
            ...
            emb = (
                model.encode_batch(
                    torch.tensor(y[np.newaxis, :], device=device),
                    normalize=self.cluster,
                )
                .cpu()
                .numpy()
                .squeeze(axis=1)
