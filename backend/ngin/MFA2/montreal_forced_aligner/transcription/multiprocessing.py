...
def compose_lg(
    dictionary_path: Path, small_g_path: Path, lg_path: Path, log_file: TextIO
) -> None:
    """
    Compose an LG.fst
...
def compose_hclg(
    ...
    ha_out_disambig = hclga_path.with_stem(
        "disambig_tid" + hclga_path.stem.split(".")[-1]
    )
...
def compose_g(
    arpa_path: Path, words_path: Path, g_path: Path, log_file: TextIO
) -> None:
    """
...
class CreateHclgFunction(KaldiFunction):
    ...
    def _run(self) -> typing.Generator[typing.Tuple[bool, str]]:
        ...
        with mfa_open(self.log_path, "w") as log_file:
            ...
            clg_path = hclg_path.with_stem(
                f"CLG_{context_width}_{central_pos}.{self.job_name}"
            )
            ...
            if not os.path.exists(medium_g_path):
                ...
                compose_g(
                    self.medium_arpa_path, self.words_path, medium_g_path, log_file
                )
                yield 1
