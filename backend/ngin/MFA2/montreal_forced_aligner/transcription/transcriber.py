...
class TranscriberMixin(CorpusAligner):
        ...
        with self.session() as session:
            for j in self.jobs:
                ...
                for s in speakers:
                    ...
                    speaker_paths[s.id] = os.path.join(
                        self.data_directory, f"{s.id}.txt"
                    )
                arguments.append(
                    TrainSpeakerLmArguments(
                        j.id,
            ...
            if GLOBAL_CONFIG.use_mp:
                ...
                for args in arguments:
                    ...
                    for _ in function.run():
                        pbar.update(1)
        logger.debug(
            f"Compiling speaker language models took {time.time() - begin:.3f} seconds"
        )
    ...
    def lm_rescore(self) -> None:
        ...
        if GLOBAL_CONFIG.use_mp:
            ...
            for args in self.lm_rescore_arguments():
                function = LmRescoreFunction(args)
                with tqdm(
                    total=GLOBAL_CONFIG.num_jobs, disable=GLOBAL_CONFIG.quiet
                ) as pbar:
    ...
    def train_phone_lm(self):
        ...
        with mfa_open(log_path, "w") as log_file:
            ...
            log_file.flush()
            bigram_fst = model.construct_bigram_fst(
    ...
    def evaluate_transcriptions(self) -> Tuple[float, float]:
        ...
        logger.info(
            f"SER: {100 * ser:.2f}%, WER: {100 * wer:.2f}%, CER: {100 * cer:.2f}%"
        )
    ...
    def calc_initial_fmllr(self) -> None:
        ...
        with tqdm(total=self.num_speakers, disable=GLOBAL_CONFIG.quiet) as pbar:
            ...
            if sum_errors:
                logger.warning(
                    f"{sum_errors} utterances had errors on calculating fMLLR."
                )
                ...
                arguments.append(
                    PerSpeakerDecodeArguments(
                        ...
                        self.working_log_directory.joinpath(
                            f"per_speaker_decode.{j.id}.log"
                        ),
                        self.model_directory,
                        feat_strings,
                        j.construct_path_dictionary(
                            self.working_directory, "lat", "ark"
                        ),
                        self.model_path,
                        self.disambiguation_symbols_int_path,
                        self.decode_options,
    ...
    def lm_rescore_arguments(self) -> List[LmRescoreArguments]:
        ...
        return [
            LmRescoreArguments(
                ...
                j.construct_path_dictionary(
                    self.working_directory, "lat.rescored", "ark"
                ),
                j.construct_dictionary_dependent_paths(
                    self.model_directory, "G_small", "fst"
                ),
                j.construct_dictionary_dependent_paths(
                    self.model_directory, "G_med", "fst"
                ),
            )
            for j in self.jobs
        ]
    ...
    def carpa_lm_rescore_arguments(self) -> List[CarpaLmRescoreArguments]:
        ...
        return [
            CarpaLmRescoreArguments(
                ...
                j.construct_path_dictionary(
                    self.working_directory, "lat.carpa.rescored", "ark"
                ),
                j.construct_dictionary_dependent_paths(
                    self.model_directory, "G_med", "fst"
                ),
                j.construct_dictionary_dependent_paths(
                    self.model_directory, "G", "carpa"
                ),
            )
            for j in self.jobs
        ]
        ...
        for j in self.jobs:
            ...
            if workflow is not WorkflowType.phone_transcription:
                for d in j.dictionaries:
                    ...
                    hclg_paths[d.id] = os.path.join(
                        self.model_directory, f"HCLG.{d.id}.fst"
                    )
            ...
            arguments.append(
                LatGenFmllrArguments(
                    ...
                    hclg_paths,
                    j.construct_path_dictionary(
                        self.working_directory, "lat.tmp", "ark"
                    ),
                )
            )
...
class Transcriber(TranscriberMixin, TopLevelMfaWorker):
    ...
    def create_hclgs(self) -> None:
        ...
        if GLOBAL_CONFIG.use_mp:
            ...
            for i, args in enumerate(dict_arguments):
                ...
                p.start()
            with tqdm(
                total=len(dict_arguments) * 7, disable=GLOBAL_CONFIG.quiet
            ) as pbar:
                while True:
                    try:
                        result = return_queue.get(timeout=1)
            ...
            for args in dict_arguments:
                function = CreateHclgFunction(args)
                with tqdm(
                    total=len(dict_arguments), disable=GLOBAL_CONFIG.quiet
                ) as pbar:
                    for result in function.run():
                        ...
                        if result:
                            ...
                            logger.warning(
                                f"There was an error in generating {hclg_path}"
                            )
                        pbar.update(1)
        error_logs = []
    ...
    def create_decoding_graph(self) -> None:
        ...
        if not os.path.exists(small_arpa_path) or not os.path.exists(medium_arpa_path):
            ...
            with mfa_open(big_arpa_path, "r") as inf, mfa_open(
                new_carpa_path, "w"
            ) as outf:
                for line in inf:
            ...
            if not os.path.exists(small_arpa_path):
                ...
                subprocess.call(
                    ["ngramprint", "--ARPA", small_mod_path, small_arpa_path]
                )
            ...
            if not os.path.exists(medium_arpa_path):
                ...
                subprocess.call(
                    ["ngramprint", "--ARPA", med_mod_path, medium_arpa_path]
                )
        try:
            self.create_hclgs()
        except Exception as e:
    ...
    def parse_parameters(
        ...
        if args.get("language_model_weight", None) is not None:
            ...
            global_params["max_language_model_weight"] = (
                args["language_model_weight"] + 1
            )
