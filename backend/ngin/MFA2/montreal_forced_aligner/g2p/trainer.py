...
class PyniniTrainerMixin:
    ...
    def align_path(self) -> Path:
        ...
        return self.working_directory.joinpath(
            f"{self.data_source_identifier}.align.fst"
        )
    ...
    def afst_path(self) -> Path:
        ...
        return self.working_directory.joinpath(
            f"{self.data_source_identifier}.afst.far"
        )
    ...
    def input_path(self) -> Path:
        ...
        return self.working_directory.joinpath(
            f"input_{self.data_source_identifier}.txt"
        )
    ...
    def output_path(self) -> Path:
        ...
        return self.working_directory.joinpath(
            f"output_{self.data_source_identifier}.txt"
        )
    ...
    def _alignments(self) -> None:
        ...
        if not os.path.exists(self.align_path):
            ...
            (best_fst, best_likelihood) = min(
                fst_likelihoods.items(), key=operator.itemgetter(1)
            )
...
class PyniniTrainer(
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._data_source = os.path.splitext(
            os.path.basename(kwargs["dictionary_path"])
        )[0]
        super().__init__(**kwargs)
        self._fst_path = None
        self._sym_path = None
    ...
    def initialize_training(self) -> None:
        ...
        self.output_token_type = pynini.SymbolTable.read_text(
        ...
        with self.session() as session:
            ...
            with mfa_open(self.input_path, "w") as inf, mfa_open(
                self.output_path, "w"
            ) as outf:
