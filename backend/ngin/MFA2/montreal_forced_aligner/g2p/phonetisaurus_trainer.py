    ...
    def run(self) -> None:
        ...
        try:
            ...
            with mfa_open(self.log_path, "w") as log_file, sqlalchemy.orm.Session(
                ...
                for current_index, (input, output) in enumerate(
                    self.data_generator(session)
                ):
                    ...
                    try:
                                ...
                                if self.insertions:
                                    for input_range in range(1, self.input_order + 1):
                                        if i + input_range <= len(input):
                                            ...
                                            input_string = self.seq_sep.join(
                                                subseq_input
                                            )
                                            if (
                                                input_range > 1
                                                and input_string
                                                not in valid_input_ngrams
                                            ):
                                                continue
                                            symbol = self.s1s2_sep.join(
                                                [input_string, self.skip]
                                            )
                                            ilabel = symbol_table.find(symbol)
                                            if ilabel == pywrapfst.NO_LABEL:
                                                ilabel = symbol_table.add_symbol(symbol)
                                            ostate = (i + input_range) * (
                                                len(output) + 1
                                            ) + j
                                            fst.add_arc(
                                                istate,
                        ...
                        fst.set_start(0)
                        fst.set_final(
                            final_state, pywrapfst.Weight.one(fst.weight_type())
                        )
...
class ExpectationWorker(mp.Process):
    ...
    def run(self) -> None:
        ...
        symbol_table = pywrapfst.SymbolTable.read_text(
            self.far_path.with_suffix(".syms")
        )
        symbol_mapper = {}
        data = {}
        count = 0
                ...
                for state_id in fst.states():
                    for arc in fst.arcs(state_id):
                        gamma = pywrapfst.divide(
                            pywrapfst.times(
                                pywrapfst.times(alpha[state_id], arc.weight),
                                beta[arc.nextstate],
                            ),
                            beta[0],
                        )
...
class MaximizationWorker(mp.Process):
    ...
    def run(self) -> None:
        ...
        try:
                    ...
                    if self.penalize_em:
                        if m2m.grapheme_order > 1 or m2m.phone_order > 1:
                            weight = pywrapfst.Weight(
                                "log", float(weight) * m2m.total_order
                            )
                        if (
                            weight == pywrapfst.Weight.zero("log")
                            or float(weight) == numpy.inf
                        ):
                            weight = pywrapfst.Weight("log", 99)
                    alignment_model[symbol_table.find(m2m.symbol)] = weight
            far_reader = pywrapfst.FarReader.open(self.far_path)
            far_writer = pywrapfst.FarWriter.create(
                self.far_path.with_suffix(self.far_path.suffix + ".temp"),
                arc_type="log",
            )
            while not far_reader.done():
                if self.stopped.stop_check():
            ...
            os.rename(
                self.far_path.with_suffix(self.far_path.suffix + ".temp"), self.far_path
            )
        except Exception as e:
            self.stopped.stop()
            self.return_queue.put(e)
...
class AlignmentExporter(mp.Process):
    ...
    def __init__(
        self, return_queue: mp.Queue, stopped: Stopped, args: AlignmentExportArguments
    ):
        mp.Process.__init__(self)
        self.return_queue = return_queue
        self.stopped = stopped
            ...
            with mfa_open(one_best_path, "w") as f:
                while not far_reader.done():
                    ...
                    if fst.num_states() == 0:
                        ...
                        continue
                    tfst = pynini.arcmap(
                        pynini.Fst.read_from_string(fst.write_to_string()),
                        map_type="to_std",
                    )
                    if self.penalize:
                        for state in tfst.states():
                            ...
                            del maiter
                    pfst = (
                        rewrite.lattice_to_dfa(tfst, True, 8)
                        .project("output")
                        .rmepsilon()
                    )
                    ...
                    if pfst.start() != pywrapfst.NO_SYMBOL:
                        ...
                        pfst = (
                            rewrite.lattice_to_dfa(tfst, False, 8)
                            .project("output")
                            .rmepsilon()
                        )
                        path = pynini.shortestpath(pfst)
...
class NgramCountWorker(mp.Process):
    ...
    def __init__(
        self, return_queue: mp.Queue, stopped: Stopped, args: NgramCountArguments
    ):
        mp.Process.__init__(self)
        self.return_queue = return_queue
        self.stopped = stopped
...
class PhonetisaurusTrainerMixin:
        ...
        with tqdm(
                ...
                for symbol, weight in weights.items():
                    ...
                    if symbol not in symbols:
                        ...
                        if left_side == self.skip:
                            ...
                            left_side_order = 1 + left_side.count(
                                self.sequence_separator
                            )
                        if right_side == self.skip:
                            right_side_order = 0
                        else:
                            right_side_order = 1 + right_side.count(
                                self.sequence_separator
                            )
                        ...
                        symbols[symbol]["weight"] = pynini.plus(
            ...
            if error_list:
                for v in error_list:
                    raise v
            logger.debug(
                f"Total of {len(symbols)} symbols, initial total: {self.total}"
            )
    ...
    def maximization(self, last_iteration=False) -> float:
        ...
        with self.session(autoflush=False, autocommit=False) as session:
            session.query(M2MSymbol).update(
                {"weight": M2MSymbol.weight - float(self.total)},
    ...
    def expectation(self) -> None:
        ...
        with self.session() as session:
            bulk_update(
                session,
                M2MSymbol,
                [{"id": k, "weight": float(v)} for k, v in mappings.items()],
            )
            session.commit()
            ...
            for state in ngram_fst.states():
                ...
                while not maiter.done():
                    ...
                    try:
                        ...
                        arc = pywrapfst.Arc(
                            g_symbol, p_symbol, arc.weight, arc.nextstate
                        )
                        maiter.set_value(arc)
                    except ValueError:
    ...
    def train_alignments(self) -> None:
        ...
        if os.path.exists(self.alignment_model_path):
            ...
            self.symbol_table = pynini.SymbolTable.read_text(
                self.alignment_symbols_path
            )
            return
    ...
    def compute_initial_ngrams(self) -> None:
        ...
        for line in print_proc.stdout:
            ...
            ngram = self.sequence_separator.join(
                x for x in line if x not in {"<s>", "</s>"}
            )
    ...
    def initialize_training(self) -> None:
        ...
        with self.session() as session:
            ...
            self.g2p_num_training_words = (
                session.query(Word2Job.word_id)
                .filter(Word2Job.training == True)
                .count()  # noqa
            )
            logger.debug(
                f"Graphemes in training data: {sorted(self.g2p_training_graphemes)}"
            )
            logger.debug(f"Phones in training data: {sorted(self.g2p_training_phones)}")
            logger.debug(
                f"Averages phones per grapheme: {phone_count / grapheme_count}"
            )

            if (
                self.sequence_separator
                in self.g2p_training_phones | self.g2p_training_graphemes
            ):
                raise PhonetisaurusSymbolError(
                    self.sequence_separator, "sequence_separator"
                )
            ...
            if self.evaluation_mode:
                ...
                logger.debug(
                    f"Phones in validation data: {sorted(self.g2p_validation_phones)}"
                )
                grapheme_diff = sorted(
                    self.g2p_validation_graphemes - self.g2p_training_graphemes
                )
                phone_diff = sorted(
                    self.g2p_validation_phones - self.g2p_training_phones
                )
                if grapheme_diff:
                    logger.debug(
