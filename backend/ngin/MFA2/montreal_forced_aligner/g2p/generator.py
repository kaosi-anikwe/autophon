...
class Rewriter:
    ...
    def __call__(self, graphemes: str) -> List[str]:  # pragma: no cover
        ...
        if " " in graphemes:
            ...
            hypotheses = sorted(
                set(" ".join(x) for x in itertools.product(*hypotheses))
            )
        else:
            ...
            for j in range(1, self.grapheme_order + 1):
                if i + j <= len(word):
                    ...
                    if ilabel != pywrapfst.NO_LABEL:
                        fst.add_arc(
                            start_state, pywrapfst.Arc(ilabel, ilabel, one, i + j)
                        )
                    if i + j >= max_state:
                        max_state = i + j
...
class PyniniGenerator(G2PTopLevelMixin):
    ...
    def __init__(
        self, g2p_model_path: Path = None, strict_graphemes: bool = False, **kwargs
    ):
    ...
    def setup(self):
        ...
        if self.g2p_model.meta["architecture"] == "phonetisaurus":
            ...
            self.input_token_type = pynini.SymbolTable.read_text(
    ...
    def compute_validation_errors(
        ...
        for word, gold_pronunciations in gold_values.items():
            if word not in hypothesis_values:
                incorrect += 1
                gold_length = statistics.mean(
                    len(x.split()) for x in gold_pronunciations
                )
            ...
            for h in hyp:
                ...
                indices.append(word)
                to_comp.append(
                    (gold_pronunciations, hyp)
                )  # Multiple hypotheses to compare
            logger.debug(
...
class PyniniCorpusGenerator(PyniniGenerator, TextCorpusMixin, TopLevelMfaWorker):
    ...
    def export_file_pronunciations(self, output_file_path: Path):
        ...
        with tqdm(total=self.num_utterances, disable=GLOBAL_CONFIG.quiet) as pbar:
            ...
            for utt_id, pronunciation in run_kaldi_function(
                ...
                update_mapping.append(
                    {"id": utt_id, "transcription_text": pronunciation}
                )
        ...
        logger.debug(
            f"Processed {self.num_utterances} in {time.time() - begin:.3f} seconds"
        )
