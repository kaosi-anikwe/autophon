...
class TokenizerAlignmentInitWorker(AlignmentInitWorker):
    ...
    def data_generator(self, session):
        grapheme_table = pywrapfst.SymbolTable.read_text(
            self.far_path.with_name("graphemes.syms")
        )
        ...
        for (text,) in query:
            tokenized = [
                x if grapheme_table.member(x) else "<unk>" for x in text.split()
            ]
                        ...
                        for i in range(len(input) + 1):
                            for j in range(len(output) + 1):
                                ...
                                for input_range in range(1, self.input_order + 1):
                                    for output_range in range(
                                        input_range, self.output_order + 1
                                    ):
...
class TokenizerMixin(
    AcousticCorpusMixin, G2PTrainer, DictionaryMixin, TopLevelMfaWorker
):
...
class PhonetisaurusTokenizerTrainer(PhonetisaurusTrainerMixin, TokenizerMixin):
    ...
    def finalize_training(self) -> None:
        ...
        shutil.copyfile(
            self.grapheme_symbols_path, self.working_directory.joinpath("input.syms")
        )
        shutil.copyfile(
            self.phone_symbols_path, self.working_directory.joinpath("output.syms")
        )
        if self.evaluation_mode:
    ...
    def export_model(self, output_model_path: Path) -> None:
        ...
        model = TokenizerModel.empty(
            output_model_path.stem, root_directory=models_temp_dir
        )
