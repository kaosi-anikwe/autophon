...
class ValidationMixin:
    ...
    def analyze_setup(self) -> None:
        ...
        with self.session() as session:
            ...
            total_duration = session.query(
                sqlalchemy.func.sum(Utterance.duration)
            ).scalar()
        ...
        logger.debug(
            f"Ignored count calculation took {time.time() - begin:.3f} seconds"
        )
        ...
        if len(self.textgrid_read_errors):
            logger.error(
                f"{len(self.textgrid_read_errors)} read errors for TextGrid files"
            )
    ...
    def analyze_missing_features(self) -> None:
        ...
        with self.session() as session:
            utterances = (
                session.query(
                    File.name, File.relative_path, Utterance.begin, Utterance.end
                )
                .join(Utterance.file)
    ...
    def compile_information(self) -> None:
        ...
        if not avg_like_frames:
            ...
            if too_short_count:
                logger.error(
                    too_short_count,
                    f"{too_short_count} utterances were too short to be aligned",
                )
            else:
            ...
            logger.debug(
                f"Average per frame likelihood for alignment: {average_log_like}"
            )
        logger.debug(
            f"Compiling information took {time.time() - compile_info_begin:.3f} seconds"
        )
    ...
    def test_utterance_transcriptions(self) -> None:
        ...
        try:
            ...
            out_path = os.path.join(
                self.output_directory, "transcription_evaluation.csv"
            )
...
class TrainingValidator(TrainableAligner, ValidationMixin):
    ...
    def setup(self) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Wrote lexicon information in {time.time() - begin:.3f} seconds"
            )
