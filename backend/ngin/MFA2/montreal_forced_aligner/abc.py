...
class DatabaseMixin(TemporaryDirectoryMixin, metaclass=abc.ABCMeta):
    ...
    def initialize_database(self) -> None:
        ...
        if GLOBAL_CONFIG.current_profile.use_postgres:
            with self.db_engine.connect() as conn:
                ...
                conn.execute(
                    sqlalchemy.text("CREATE EXTENSION IF NOT EXISTS pg_stat_statements")
                )
                conn.commit()
    ...
    def create_new_current_workflow(
        self, workflow_type: WorkflowType, name: str = None
    ):
        ...
        with self.session() as session:
            ...
            new_workflow = (
                ...
                .filter(CorpusWorkflow.name == name)
                .first()
            )
            if not new_workflow:
    ...
    def set_current_workflow(self, identifier):
        ...
        with self.session() as session:
            ...
            wf = (
                session.query(CorpusWorkflow)
                .filter(CorpusWorkflow.name == identifier)
                .first()
            )
            wf.current = True
...
class TopLevelMfaWorker(MfaWorker, TemporaryDirectoryMixin, metaclass=abc.ABCMeta):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if skipped:
            logger.warning(
                f"Skipped the following configuration keys: {comma_join(skipped)}"
            )
    ...
    def worker_config_path(self) -> str:
        ...
        return os.path.join(
            self.output_directory, f"{self.data_source_identifier}.yaml"
        )
    ...
    def cleanup(self) -> None:
        ...
        try:
            ...
            if self.dirty:
                ...
                logger.info(
                    f"Done! Everything took {time.time() - self.start_time:.3f} seconds"
                )
