...
def bulk_update(
    ...
    if GLOBAL_CONFIG.current_profile.use_postgres:
        session.execute(
            sqlalchemy.text(f"ALTER TABLE {table.__tablename__} DISABLE TRIGGER all")
        )
        session.commit()
    ...
    if GLOBAL_CONFIG.current_profile.use_postgres:
        session.execute(
            sqlalchemy.text(f"ALTER TABLE {table.__tablename__} ENABLE TRIGGER all")
        )
        session.commit()
...
class Word(MfaSqlBase):
    ...
    dictionary_id = Column(
        Integer, ForeignKey("dictionary.id"), nullable=False, index=True
    )
...
class RuleApplication(MfaSqlBase):
    ...
    pronunciation_id = Column(
        ForeignKey("pronunciation.id", ondelete="CASCADE"), primary_key=True
    )
    rule_id = Column(
        ForeignKey("phonological_rule.id", ondelete="CASCADE"), primary_key=True
    )
...
class Speaker(MfaSqlBase):
    ...
    dictionary_id = Column(
        Integer, ForeignKey("dictionary.id"), nullable=True, index=True
    )
            ...
            for utterance in self.utterances:
                ...
                if save_transcription:
                    ...
                    if tiers[utterance.speaker.name].entries:
                        if (
                            tiers[utterance.speaker.name].entries[-1].end
                            > utterance.begin
                        ):
                            utterance.begin = (
                                tiers[utterance.speaker.name].entries[-1].end
...
class Utterance(MfaSqlBase):
    ...
    def phone_intervals_for_workflow(
        ...
        """
        return [
            x.as_ctm() for x in self.phone_intervals if x.workflow_id == workflow_id
        ]
    ...
    @classmethod
    def from_data(
        cls, data: UtteranceData, file: File, speaker: int, frame_shift: int = None
    ):
        """
...
class PhoneInterval(MfaSqlBase):
    ...
    word_interval_id = Column(
        Integer,
        ForeignKey("word_interval.id", ondelete="CASCADE"),
        index=True,
        nullable=True,
    )
    ...
    workflow_id = Column(
        Integer,
        ForeignKey("corpus_workflow.id", ondelete="CASCADE"),
        index=True,
        nullable=False,
    )
    ...
    __table_args__ = (
        sqlalchemy.Index(
            "phone_utterance_workflow_index", "utterance_id", "workflow_id"
        ),
    )
    ...
    def as_ctm(self) -> CtmInterval:
        ...
        """
        return CtmInterval(
            self.begin, self.end, self.phone.phone, confidence=self.phone_goodness
        )
...
class WordInterval(MfaSqlBase):
    ...
    pronunciation_id = Column(
        Integer, ForeignKey("pronunciation.id"), index=True, nullable=True
    )
    ...
    __table_args__ = (
        sqlalchemy.Index(
            "word_utterance_workflow_index", "utterance_id", "workflow_id"
        ),
    )
...
class Job(MfaSqlBase):
    ...
    def construct_online_feature_proc_string(self):
        feat_path = self.construct_path(
            self.corpus.current_subset_directory, "feats", "scp"
        )
    ...
    def construct_feature_proc_string(
        ...
        feat_path = self.construct_path(
            self.corpus.current_subset_directory,
            "feats",
            "scp",
