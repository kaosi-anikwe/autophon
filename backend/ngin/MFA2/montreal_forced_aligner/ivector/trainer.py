...
class DubmTrainer(IvectorModelTrainingMixin):
    ...
    def _trainer_initialization(
        self, initial_alignment_directory: Optional[str] = None
    ) -> None:
        ...
        with self.session() as session, mfa_open(log_path, "w") as log_file:
            ...
            num_gauss_init = int(
                self.initial_gaussian_proportion * int(self.num_gaussians)
            )
            self.iteration = 1
            if self.use_alignment_features and alignment_workflow is not None:
                model_path = os.path.join(
                    alignment_workflow.working_directory, "final.mdl"
                )
                occs_path = os.path.join(
                    alignment_workflow.working_directory, "final.occs"
                )
                ...
                feature_string = feature_string.replace(
                    str(job.corpus.current_subset_directory),
                    str(job.corpus.data_directory),
                )
                gmm_init_proc = subprocess.Popen(
                    [
        ...
        for j in self.jobs:
            arguments.append(
                AccIvectorStatsArguments(
                    ...
                    os.path.join(
                        self.working_log_directory,
                        f"ivector_acc.{self.iteration}.{j.id}.log",
                    ),
                    self.feature_options,
        ...
        if not os.path.exists(self.ie_path):
            with mfa_open(log_path, "w") as log_file:
                subprocess.check_call(
                    [
                        thirdparty_binary("gmm-global-to-fgmm"),
                        diag_ubm_path,
                        full_ubm_path,
                    ],
                    stderr=log_file,
    ...
    def acc_ivector_stats(self) -> None:
        ...
        with tqdm(
            ...
            for _ in run_kaldi_function(
                AccIvectorStatsFunction, arguments, pbar.update
            ):
                pass
        ...
        with mfa_open(log_path, "w") as log_file:
            ...
            for line in extractor_est_proc.stderr:
                ...
                m = re.match(
                    r"LOG.*variance explained by the iVectors is (?P<variance>[0-9.]+)\.",
                    line,
                )
                if m:
...
class TrainableIvectorExtractor(
    IvectorCorpusMixin, TopLevelMfaWorker, ModelExporterMixin
):
    """
    ...
    def __init__(
        self, training_configuration: List[Tuple[str, Dict[str, Any]]] = None, **kwargs
    ):
        self.param_dict = {
            k: v
