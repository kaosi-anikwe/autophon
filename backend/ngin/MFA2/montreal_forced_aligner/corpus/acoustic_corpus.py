...
class AcousticCorpusMixin(CorpusMixin, FeatureConfigMixin, metaclass=ABCMeta):
    ...
    def has_any_ivectors(self) -> bool:
        with self.session() as session:
            check = (
                ...
                .filter(
                    sqlalchemy.or_(
                        Corpus.ivectors_calculated == True,
                        Corpus.xvectors_loaded == True,  # noqa
                    )
                )
                .limit(1)
                .first()
    ...
    def load_reference_alignments(self, reference_directory: Path) -> None:
        ...
        with tqdm(
            ...
            for root, _, files in os.walk(reference_directory, followlinks=True):
                ...
                for f in files:
                    if f.endswith(".TextGrid"):
                        ...
                        file_id = (
                            session.query(File.id).filter_by(name=file_name).scalar()
                        )
                        if not file_id:
                            continue
            ...
            if new_phones:
                ...
                session.commit()
            session.execute(
                sqlalchemy.insert(PhoneInterval.__table__), reference_intervals
            )
    ...
    def generate_final_features(self) -> None:
        ...
        logger.debug(
            f"Generating final features took {time.time() - time_begin:.3f} seconds"
        )
    ...
    def generate_features(self) -> None:
        ...
        with self.session() as session:
            ...
            feature_check = (
                ...
                .filter(Utterance.features != None)
                .first()  # noqa
                is not None
            )
        ...
        with tqdm(total=self.num_speakers, disable=GLOBAL_CONFIG.quiet) as pbar:
            for speaker_id, min_f0, max_f0 in run_kaldi_function(
                ...
                update_mapping.append(
                    {"id": speaker_id, "min_f0": min_f0, "max_f0": max_f0}
                )
        with self.session() as session:
    ...
    def calc_cmvn(self) -> None:
        ...
        with self.session() as session:
            ...
            for j in self.jobs:
                ...
                with mfa_open(
                    j.construct_path(self.split_directory, "cmvn", "scp"), "w"
                ) as f:
    ...
    def _load_corpus_from_source(self) -> None:
        ...
        if self.audio_directory and os.path.exists(self.audio_directory):
            ...
            for root, _, files in os.walk(self.audio_directory, followlinks=True):
                ...
                exts.wav_files = {
                    k: os.path.join(root, v) for k, v in exts.wav_files.items()
                }
        ...
        with self.session() as session:
            ...
            for root, _, files in os.walk(self.corpus_directory, followlinks=True):
                exts = find_exts(files)
                relative_path = (
                    root.replace(str(self.corpus_directory), "")
                    .lstrip("/")
                    .lstrip("\\")
                )
                if self.stopped.stop_check():
        ...
        logger.debug(
            f"Parsed corpus directory in {time.time() - begin_time:.3f} seconds"
        )
