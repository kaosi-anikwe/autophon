...
class IvectorCorpusMixin(AcousticCorpusMixin, IvectorConfigMixin):
    ...
    def _write_ivectors(self) -> None:
        ...
        with self.session() as session, mfa_open(
            self.utterance_ivector_path, "w"
        ) as outf:
            utterances = (
                session.query(Utterance.kaldi_id, Utterance.ivector_ark)
                .join(Utterance.speaker)
                .filter(
                    Utterance.ivector_ark != None, Speaker.name != "MFA_UNKNOWN"
                )  # noqa,
            )
    ...
    def extract_ivectors(self) -> None:
        ...
        with tqdm(total=self.num_utterances, disable=GLOBAL_CONFIG.quiet) as pbar:
            for _ in run_kaldi_function(
                ExtractIvectorsFunction, arguments, pbar.update
            ):
                pass
    ...
    def transform_ivectors(self):
        ...
        plda_path = (
            self.adapted_plda_path
            if os.path.exists(self.adapted_plda_path)
            else self.plda_path
        )
    ...
    def collect_utterance_ivectors(self) -> None:
        ...
        with self.session() as session, tqdm(
            ...
            update_mapping = {}
            for j in self.jobs:
                ivector_scp_path = j.construct_path(
                    self.split_directory, "ivectors", "scp"
                )
