        ...
        for root, _, files in os.walk(self.corpus_directory, followlinks=True):
            ...
            for file_name in exts.identifiers:
                ...
                if wav_path is None:
                    continue
                self.job_queue.put(
                    (file_name, wav_path, transcription_path, relative_path)
                )
...
class CorpusProcessWorker(mp.Process):
    ...
    def run(self) -> None:
        ...
        while True:
            try:
                file_name, wav_path, text_path, relative_path = self.job_q.get(
                    timeout=1
                )
            except Empty:
...
class NormalizeTextFunction(KaldiFunction):
    ...
    def compile_regexes(self) -> None:
        ...
        if self.brackets:
            word_break_set = (
                non_word_character_set
                | set(self.clitic_markers)
                | set(self.compound_markers)
            )
        ...
        word_break_character_set = make_re_character_set_safe(
            non_word_character_set, [r"\s"]
        )
        ...
        if len(self.clitic_markers) >= 1:
            ...
            non_clitic_punctuation_set = make_re_character_set_safe(
                non_clitic_punctuation
            )
    ...
    def _dictionary_sanitize(self, session):
        ...
        for d in dictionaries:
            ...
            if self.clitic_marker is not None:
                ...
                if initial_clitics:
                    initial_clitic_regex = re.compile(
                        rf"^({'|'.join(initial_clitics)})(?=\w)"
                    )
                if final_clitics:
                    final_clitic_regex = re.compile(
                        rf"(?<=\w)({'|'.join(final_clitics)})$"
                    )
    ...
    def _run(self) -> typing.Generator[typing.Tuple[int, float]]:
        ...
        with Session(self.db_engine()) as session:
            dict_count = (
                session.query(Dictionary).join(Dictionary.words).limit(1).count()
            )
