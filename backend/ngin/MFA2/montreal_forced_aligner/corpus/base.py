...
class CorpusMixin(MfaWorker, DatabaseMixin, metaclass=ABCMeta):
    ...
    def jobs(self) -> typing.List[Job]:
        if not self._jobs:
            with self.session() as session:
                ...
                if c.current_subset:
                    jobs = jobs.filter(
                        Job.utterances.any(Utterance.in_subset == True)
                    )  # noqa
                jobs = jobs.filter(
                    Job.utterances.any(Utterance.ignored == False)
                )  # noqa
                self._jobs = jobs.all()
        return self._jobs
    ...
    def _write_spk2utt(self) -> None:
        ...
        output_mapping(
            utt2spk_data, self.corpus_output_directory.joinpath("utt2spk.scp")
        )
    ...
    def create_corpus_split(self) -> None:
        ...
        with self.session() as session, tqdm(
            ...
            for _ in run_kaldi_function(
                ExportKaldiFilesFunction, arguments, pbar.update
            ):
                pass
    ...
    def add_utterance(
        self, utterance: UtteranceData, session: Session = None
    ) -> Utterance:
        """
        ...
        speaker_obj = (
            session.query(Speaker).filter_by(name=utterance.speaker_name).first()
        )
        if not speaker_obj:
            dictionary = None
    ...
    def initialize_jobs(self) -> None:
        ...
        with self.session() as session:
            if session.query(
                sqlalchemy.sql.exists().where(Utterance.job_id > 1)
            ).scalar():
            ...
            if (
                ...
                session.query(Job).filter(
                    Job.id > GLOBAL_CONFIG.current_profile.num_jobs
                ).delete()
            ...
            if GLOBAL_CONFIG.current_profile.single_speaker:
                utts_per_job = int(
                    self.num_utterances / GLOBAL_CONFIG.current_profile.num_jobs
                )
                if utts_per_job == 0:
                    utts_per_job = 1
                for i, j in enumerate(jobs):
                    update_mappings.extend(
                        {"id": u, "job_id": j.id}
                        for u in range(
                            (utts_per_job * i) + 1, (utts_per_job * (i + 1)) + 1
                        )
                    )
            ...
            if session.query(Dictionary2Job).count() == 0:
                ...
                for job_id, dict_id in (
                    ...
                    if not dict_id:
                        continue
                    dict_job_mappings.append(
                        {"job_id": job_id, "dictionary_id": dict_id}
                    )
    ...
    def _finalize_load(self, session: Session, import_data: DatabaseImportData):
        ...
        with session.begin_nested():
            ...
            if import_data.speaker_objects:
                session.execute(
                    sqlalchemy.insert(Speaker.__table__), import_data.speaker_objects
                )
            if import_data.file_objects:
                session.execute(
                    sqlalchemy.insert(File.__table__), import_data.file_objects
                )
            ...
            if import_data.sound_file_objects:
                session.execute(
                    sqlalchemy.insert(SoundFile.__table__),
                    import_data.sound_file_objects,
                )
            ...
            if import_data.utterance_objects:
                session.execute(
                    sqlalchemy.insert(Utterance.__table__),
                    import_data.utterance_objects,
                )
            session.flush()
    ...
    def normalize_text(self) -> None:
        ...
        with tqdm(
            ...
            has_words = (
                session.query(Dictionary).filter(Dictionary.name == "unknown")
                is not None
            )
            words = session.query(
        ...
        with self.session() as session:
            ...
            if num_dictionaries > 1:
                ...
                for dict_id in getattr(self, "dictionary_lookup", {}).values():
                    ...
                    logger.debug(
                        f"For {dict_id}, total number of utterances is {num_utts}"
                    )
            ...
            with self.session() as session, tqdm(
                ...
                arguments = [
                    ExportKaldiFilesArguments(
                        j.id, self.db_string, None, subset_directory, False
                    )
                    for j in self._jobs
                ]
    ...
    def num_speakers(self) -> int:
        ...
        if self._num_speakers is None:
            with self.session() as session:
                self._num_speakers = session.query(
                    sqlalchemy.func.count(Speaker.id)
                ).scalar()
    ...
    def get_latest_workflow_run(
        self, workflow: WorkflowType, session: Session
    ) -> CorpusWorkflow:
        """
