...
class MfccFunction(KaldiFunction):
    ...
    progress_pattern = re.compile(
        r"^LOG.* Processed (?P<num_utterances>\d+) utterances"
    )
...
class FinalFeatureFunction(KaldiFunction):
    ...
    def _run(self) -> typing.Generator[int]:
        ...
        with Session(self.db_engine()) as session, mfa_open(
            ...
            job: typing.Optional[Job] = session.get(Job, self.job_name)
            feats_scp_path = job.construct_path(self.data_directory, "feats", "scp")
            temp_scp_path = job.construct_path(
                self.data_directory, "final_features", "scp"
            )
            ...
            raw_ark_path = job.construct_path(self.data_directory, "feats", "ark")
            temp_ark_path = job.construct_path(
                self.data_directory, "final_features", "ark"
            )
...
class ExtractIvectorsFunction(KaldiFunction):
    ...
    progress_pattern = re.compile(
        r"^VLOG.*Ivector norm for utterance (?P<utterance>.+) was.*"
    )
...
def plda_log_likelihood(
    train_ivector: np.ndarray,
    test_ivector: np.ndarray,
    psi: np.ndarray,
    ...
    loglike_without_class = -0.5 * (
        logdet_without
        + M_LOG_2PI * PLDA_DIMENSION
        + np.dot(sqdiff_without, variance_without)
    )
...
def plda_distance_matrix(
    ...
    for i in numba.prange(num_train):
        for j in numba.prange(num_test):
            distance_matrix[i, j] = plda_log_likelihood(
                train_ivectors[i], test_ivectors[j], psi
            )
...
def classify_plda(
    ...
    loglike_without_class = -0.5 * (
        logdet_without
        + M_LOG_2PI * PLDA_DIMENSION
        + (sqdiff_without @ variance_without)
    )
...
class PldaModel:
    ...
    def log_likelihood(
        self, train_ivector: np.ndarray, test_ivector: np.ndarray, count: int = 1
    ):
        """
        Calculate log likelihood of two ivectors belonging to the same class
    ...
    def process_ivectors(
        self, ivectors: np.ndarray, counts: np.ndarray = None
    ) -> np.ndarray:
        """
...
class ExportIvectorsFunction(KaldiFunction):
    ...
    def _run(self) -> typing.Generator[typing.Tuple[int, int, int]]:
        ...
        with sqlalchemy.orm.Session(engine) as session, mfa_open(
            ...
            ivector_scp_path = job.construct_path(
                job.corpus.split_directory, "ivectors", "scp"
            )
            ivector_ark_path = job.construct_path(
                job.corpus.split_directory, "ivectors", "ark"
            )
...
def online_feature_proc(
    ...
    if use_pitch:
        pitch_proc = compute_pitch_process(
            log_file, wav_path, subprocess.PIPE, pitch_options
        )
        pitch_copy_proc = subprocess.Popen(
            [
                thirdparty_binary("copy-feats"),
